...
exports.puckSync = function(/*id0,cx0,cy0,px0,py0,id1,cx1,cy1,px1,py1...*/){
  debug('puck sync')
...
exports.puckCreate = function(x, y, mass){
  debug('puck create [%s] %s,%s %s', world.puckIndex, x, y, mass)

  var w = settings.data.puckRadius*2
    , h = w; // square
  var puck = new Body(shapes.rect(w,h),+x,+y);
  puck.id = 'puck'+world.puckIndex++;
  var p = puck;
  puck.oncollision = function oncollision(q,c){
    debug('puck[%s] collision',puck.id);

    var hitExtra = world.extras.get(q.id)
      , hitPaddle = world.paddles.get(q.id);
    ...
    if( hitPaddle ){
      ...
      if( world.host ){
        ...
        exports.paddleHit(puck.id,hitPaddle.id,p.current[0],p.current[1],p.previous[0],p.previous[1])
  ...
  // i hope this won't give issues...
  return puck
}

exports.puckDestroy = function(id){
  debug('puck destroy',id)
...
exports.extraHit = function(extraId,puckId){
  ...
  switch(extraId){
    ...
    case 'fog':
      ...
      world.activeExtras.set('fog', {start: world.frame, end: world.frame+100} );
      break;
    ...
    case 'multiball':
      ...
      // TODO this will make the guest create two pucks
      //      one from extraHit and another from puckCreate.
      var lastPuck = exports.puckCreate(dir[0],dir[1],p.mass)
      vec.free(dir)
...
exports.obstacleCreate = function(id,x,y){
  ...
  // only add one of each at a time
  if( extra && !world.obstacles.get(id) ){
    // move it in place
    var p = vec.make(+x,+y)
    vec.copy(p,extra.current)
    vec.copy(p,extra.previous)
...
exports.pd = exports.puckDestroy;
