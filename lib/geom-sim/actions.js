...
exports.puckCreate = function(id, x, y, mass){
  debug('puck create [%s] %s,%s %s', id, x, y, mass)

  // skip pucks that already exists
  // (to avoid multiplayer hickups)
  if( world.pucks.get(id) )
    return console.warn('ignored puckCreate "%s". already exists',id);

  var w = settings.data.puckRadius*2
    , h = w; // square
  var puck = new Body(shapes.rect(w,h),+x,+y,Body.DYNAMIC | Body.BOUNCE);
  puck.id = id;
  var p = puck;
  ...
  puck.onbounds = function onbounds(b){
    ...
    if( player ){
      ...
      if( p.flags & Body.GHOST )
        console.log('added ghosting')
...
exports.obstacleCreate = function(id,x,y){
  ...
  if( extra && !world.obstacles.get(id) ){
    ...
    // set obstacle flags
    extra.flags = Body.STATIC | Body.BOUNCE;
