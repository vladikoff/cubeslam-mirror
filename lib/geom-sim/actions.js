...
exports.puckCreate = function(x, y, mass){
  ...
  puck.oncollision = function oncollision(q,c){
    ...
    // hit a paddle
    var paddleId;
    if( ~(paddleId = world.paddles.indexOf(q)) ){
      ...
      if( world.host ){
        ...
        exports.paddleHit(puck.id,paddleId,p.current[0],p.current[1],p.previous[0],p.previous[1])
        ...
        exports.puckSpeed(puck.id, Math.min(speed, world.level.maxspeed));
  ...
  puck.onbounds = function onbounds(b){
    ...
    if( player ){
      ...
      // if there's more than one puck left:
      // remove the colliding one
      if( world.pucks.length > 1 )
        exports.puckDestroy(p.id);
...
exports.puckDestroy = function(id){
  var puck = null
    , index = null;
  for (var i = 0; i < world.pucks.length; i++) {
    if( world.pucks[i].id === id ){
      puck = world.pucks[i]
      index = i
      break;
    }
  ...
  if( puck ){
    world.pucks.splice(index,1)
    world.bodies.splice(world.bodies.indexOf(puck),1)
    world.remove.push(puck)

    // send it as reliable if host
    if( world.host ){
      buffer.push('pd', world.frame)
      buffer.push.apply(buffer,arguments)
      buffer.push(EOP)
    }
  }
...
exports.paddleHit = function(puckId,paddleId,cx,cy,px,py){
  debug('paddle hit',puckId,cx,cy,px,py)
  var P = world.pucks[+puckId]
    , c = vec.make(+cx,+cy)
    , p = vec.make(+px,+py);
  ...
  // the Body "class"
  world.lastHitPucks[puckId] = +paddleId == 0 ? 'a' : 'b';

  // send it as reliable if me
  if( world.me.paddle == paddleId ){
...
exports.playerHit = function(playerId,x,y){
  ...
  if( world.host ){
    ...
    // update the score
    var score = world.players[playerId].score + 1;
    exports.score(playerId,score);

    // update the hits
    exports.hit(playerId,x);
  }
...
exports.extraHit = function(extraId,puckId){
  var p = world.pucks[+puckId];
  switch(extraId){
    ...
    case 'fog':
      ...
      world.activeExtras.push( {id: "fog", frameStart: world.frame, frameDuration: 10000} );
      break;
...
exports.bulletCreate = function(id,x,y,v){
  debug('bullet create',id,x,y,v)

  // make sure it doesn't already exist
  for(var i=0; i < world.bullets.length; i++){
    if( !world.bullets[i] )
      continue;
    if(world.bullets[i].id === id)
      return;
  }

  // define a shape of the shot (i'm thinking long and thin..)
  var shape = shapes.rect(3,20);

  // create a shot body
  var shot = new Body(shape,+x,+y);
  shot.id = id;

  // TODO add oncollision/onbounds that will destroy it
  //      (and the obstacle/extra it hits)
  shot.oncollision = function(q,c){
    debug('bullet collision',arguments)
    exports.bulletDestroy(id);
  }

  shot.onbounds = function(b){
    debug('bullet bounds',arguments)
    exports.bulletDestroy(id);
  }
  ...
  world.bodies.push(shot)
  world.bullets.push(shot)

  // send the created one
  buffer.push('bc', world.frame)
  buffer.push.apply(buffer,arguments)
  buffer.push(EOP)
}

exports.bulletDestroy = function(id){
  debug('bullet destroy',id)
  ...
  // splice world.bodies and world.shots and mark it as
  // removed so the renderer may remove it
  // world.bullets.splice(index,1)
  world.bullets[index] = null // don't splice so the length in 3d renderer is the same
  world.bodies.splice(world.bodies.indexOf(shot),1)
  world.remove.push(shot)

  // send that it has been destroyed
  buffer.push('bd', world.frame)
  buffer.push.apply(buffer,arguments)
  buffer.push(EOP)
}

exports.forceCreate = function(kind, x, y, mass){
...
exports.forceDestroy = function(kind, x, y){
...
exports.score = function(who,score){
...
exports.hit = function(who,x){
  debug('hit %s',who,x)

  world.players[who].hits.push(+x)
  world.renderer.triggerEvent("hit", {player: who});

  // send it as reliable if host
  if( world.host ){
    buffer.push('sh', world.frame)
    buffer.push.apply(buffer,arguments)
    buffer.push(EOP)
  }
}
...
exports.gotoLevel = function(i){
  debug('goto level')
  buffer.push('gl', world.frame, i, EOP)
}
...
exports.eh = exports.extraHit;
exports.ec = exports.extraCreate;
exports.ed = exports.extraDestroy;
exports.fc = exports.forceCreate;
exports.fd = exports.forceDestroy;
exports.bc = exports.bulletCreate;
exports.bd = exports.bulletDestroy;
exports.sl = exports.scoreAlive;
exports.ss = exports.score;
exports.sh = exports.hit;
