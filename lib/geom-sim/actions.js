...
exports.playerHit = function(playerId,x,y){
  ...
  if( world.host ){
    ...
    //switch so opponent get the hit stored
...
exports.extraHit = function(extraId,puckId){
  ...
  switch(extraId){
    ...
    case 'extralife':
      ...
      console.log(world.lastHitPucks[+puckId])
      if( player ) {
        console.log(world.lastHitPucks[+puckId]);
        world.renderer.triggerEvent("heal", {player: world.lastHitPucks[+puckId]});
        world.player[world.lastHitPucks[+puckId]].score = 0;
      }
      else
