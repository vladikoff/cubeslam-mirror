    ...
    // hit an extra.
      ...
      switch(q.id){
        ...
        case 'extraball':
          ...
          exports.puckCreate(q.current[0],q.current[1],p.mass)
          var lastPuck = world.pucks[world.pucks.length-1];
          var perp = vec.perp(p.velocity)
          exports.puckSpeed(lastPuck.id,perp[0],perp[1])
          vec.free(perp)
          break;
      }
...
exports.playerHit = function(playerId,x,y){
  ...
  audio.play3D("miss", new THREE.Vector3((x-.5)*-0.8,0,(1-y)*10));
}
