...
Physics.prototype = {
  ...
  goto: function(frame, world){
    ...
    if( frame < world.frame )
      return console.error('cannot rewind the physics')
    ...
    for(var j=0; j < bodies.length; j++){
      ...
      for(var k=0; k < bodies.length; k++ ){
        ...
        if( c.willIntersect ){
          ...
          if( b.hasFlags(Body.DYNAMIC) ){
            console.error("DYNAMIC?!") // not really an error just want it loud
            // TODO does this ever occur? dynamic == 0...
            var t = vec.make()
          ...
          // GHOST
          if( a.hasFlags(Body.GHOST) && !b.hasFlags(Body.STATIC) ){
            // console.log("GHOST COLLISION!")
            // just ignoring for now (maybe even skip oncollision())
            // TODO bounce off of STATIC

          // BOUNCE
          } else if( a.hasFlags(Body.BOUNCE) && b.hasFlags(Body.BOUNCE) ){
            // console.log("BOUNCE!")

            if( b.hasFlags(Body.STEER) ){
