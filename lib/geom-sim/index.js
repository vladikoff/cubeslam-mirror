...
Simulator.prototype = {
  ...
  setup: function(){
    world.players.a.paddle = this.createPaddle(.5,1,.15,.04); // 0 means "top" (i.e. height*0)
    world.players.b.paddle = this.createPaddle(.5,0,.15,.04); // 1 means "bottom" (i.e. height*1)
  ...
  create: function(){
    this.game.inputs
      ...
      .bind(this.game.inputs.UP_ARROW,'up')
  ...
  controls: function(inputs){
    if( !world.paused ){
      ...
      if( settings.data.inputType == "motion" && inputs.motion.moved ){
        ...
        world.shooting = ( inputs.motion.y < 0.3 )
        ...
        if( inputs.down('left') ){
         ...
         //activate shot
        if( world.host && (inputs.pressed('up') || inputs.pressed('fire'))){
          //TODO: remove temp-flag that trigger animation effect for now
          world.shooting = true;
        }
        else {
