...
Simulator.prototype = {
  ...
  createPaddle: function(x,y,w,h){
    var aw = settings.data.arenaWidth
    var ah = settings.data.arenaHeight
    var paddle = new Body(shapes.rect(w*aw,settings.data.paddleDepth*2),x*aw,y*ah)
  ...
  create: function(){
    this.game.inputs
      ...
      .bind(this.game.inputs.UP_ARROW,'fire')
      .bind(this.game.inputs.A,'left')
      .bind(this.game.inputs.D,'right')
      .bind(this.game.inputs.LEFT,'fire')
      .bind(this.game.inputs.W,'fire');
  ...
  destroy: function(){
    ...
    this.game.inputs
      ...
      .unbind(this.game.inputs.W)
      .unbind(this.game.inputs.LEFT_ARROW)
      .unbind(this.game.inputs.RIGHT_ARROW)
      .unbind(this.game.inputs.UP_ARROW)
      .unbind(this.game.inputs.LEFT);
  ...
  controls: function(inputs){
    if( !world.paused ){
      ...
      if( settings.data.inputType == "motion" && inputs.motion.moved ){
        ...
        if( inputs.down('left') ){
          world.keyboardSpeed += ( settings.data.keyboardSpeedMax*-1 - world.keyboardSpeed)*(1-settings.data.keyboardAccelerate*settings.data.keyboardAccelerate);
          ...
          world.keyboardSpeed += ( settings.data.keyboardSpeedMax - world.keyboardSpeed)*(1-settings.data.keyboardAccelerate*settings.data.keyboardAccelerate);
          var x = world.paddles[world.me.paddle].current[0] + world.keyboardSpeed;
          x /= settings.data.arenaWidth
          actions.paddleMove(world.me.paddle, world.host ? x : 1-x);

        } else {
          world.keyboardSpeed *= settings.data.keyboardDamping;
