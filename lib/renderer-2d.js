...
Renderer.prototype = {
  ...
  drawPaddle: function(ctx, paddle, style){
    drawPoly(ctx, paddle, this.w, this.h,true)
  ...
  drawBounds: function(ctx,bounds){
    drawPoly(ctx,bounds,1,1,true) // bounds are already scaled
  ...
  drawPointMass: function(ctx, pointMass){
    ...
    if( pointMass.links.length ){
      ...
      var r = pointMass.radius*10 || 5;
      ctx.fillRect(pointMass.current.x*w-r,pointMass.current.y*h-r,r+r,r+r)

      // draw trail
      ctx.beginPath()
      ctx.moveTo(pointMass.previous.x*w,pointMass.previous.y*h)
      ctx.lineTo(pointMass.current.x*w,pointMass.current.y*h)
      ctx.closePath()
      ctx.stroke()
    }
...
function drawPoly(ctx,polygon,w,h,withNormal,withIndex){
  ...
  ctx.fillRect(c.x*w-5,c.y*h-5,10,10);
