...
Renderer.prototype = {
  ...
  drawPaddle: function(ctx, paddle, style){
    var w = this.w
      , h = this.h;
    ctx.beginPath();
    ctx.moveTo(paddle.x*w-(paddle.width*w)/2,paddle.y*h);
    ctx.lineTo(paddle.x*w+(paddle.width*h)/2,paddle.y*h);
    ctx.closePath();
  ...
  drawInfo: function(ctx, world){
    ...
    ctx.translate(w/2,0)
    ctx.rotate(Math.PI)
    ctx.fillText(world.players.a.name,-t/2,0);
    ctx.restore()

    var t = ctx.measureText(world.players.b.name).width;
    ctx.fillText(world.players.b.name,w/2-t/2,h);
    ...
    ctx.rotate(Math.PI)
    ctx.fillText(world.players.a.score,-t/2,0);
    ctx.restore()

    var t = ctx.measureText(world.players.b.score).width;
    ctx.fillText(world.players.b.score,w/2-t/2,h-100);
  },
  ...
  render: function(world, alpha){
    ...
    var w = this.w = world.bounds.r-world.bounds.l
      , h = this.h = world.bounds.b-world.bounds.t
