...
exports.servers = { iceServers: [
  {url: 'stun:ec2-176-34-161-202.eu-west-1.compute.amazonaws.com:3478'}
]}
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4 && xhr.status == 200) {
      ...
      if (data.uris && data.uris[0] && data.username && data.password) {
        ...
        // Tidigare så har vi kört nedanstående två rader på 'turn'-eventet, alltså när TURN-config kom via Channel API:et.
        // Nu så görs detta via AJAX-requestet, och för mig så blir ajax-anropet färdigt innan första createConnection() körs,
        // så därför behöver vi inte göra om anslutningen.
        // Potentiell race condition här, men jag tror att det är lugnt.
