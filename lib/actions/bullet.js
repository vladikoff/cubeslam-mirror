...
exports.hitBullet = function(world, bullet, body){
  ...
  if( body.id == 'paddle' ){
    ...
    actions.resizePaddle(world, body.index, .5);

    console.log((body.index == world.me.paddle?"user":"opponent")+ "_paddle_shrink");

    dmaf.tell( (body.index == world.me.paddle?"user":"opponent")+ "_paddle_shrink")

    // timeout after 5s and scale back to normal
    world.tick.clearTimeout(body.data.resizeTimeout);
    body.data.resizeTimeout = world.tick.setTimeout('resizePaddle',5000,body.index,1)
  }
}
