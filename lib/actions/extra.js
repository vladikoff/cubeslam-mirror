...
exports.createRandomExtra = function(world){
  ...
  var positions = shuffle(world,world.level.extras.positions);
...
exports.createExtra = function(world, data, x, y){
  ...
  if( validExtra(data) ){
    ...
    switch(data.id){
      ...
      case 'bulletproof':
      case 'laser':
        dmaf.tell( extra.data.id + "_spawn");
        break;
      case 'ghostball':
        dmaf.tell( "ghost_ball_spawn");
        break;
      case 'extralife':
        dmaf.tell( "extra_life_spawn");
        break;
      case 'mirroredcontrols':
        dmaf.tell( "mirrored_controls_spawn");
        break;
      case 'deathball':
        dmaf.tell( "death_ball_spawn");
        break;
      case 'paddleresize':
        dmaf.tell( "paddle_resize_spawn");
        break;
      case 'timebomb':
        dmaf.tell( "time_bomb_spawn");
        break;
      }
...
exports.hitExtra = function(world, puck, extra){
  ...
  switch(id){
    ...
    case 'mirroredcontrols':
      // mirror the controls
      dmaf.tell( "mirrored_controls_activate");
    ...
    case 'bulletproof':
      dmaf.tell( "bulletproof_activate");
    ...
    case 'paddleresize':
      dmaf.tell( "paddle_resize_activate");
      var playerID = world.lastHitPucks[puck.index]
        , player = world.players[playerID]
        , paddle = world.paddles.get(player.paddle);

      // large or small?
      var scale = 2//world.random() > .5 ? 1.25 : 0.75;
    ...
    case 'timebomb':
      ...
      }.bind(null,puck,data.radius || settings.data.arenaHeight / 2),(data.time || 3)*1000)
      break;

    case 'laser':
      dmaf.tell( "laser_activate");
      var playerID = world.lastHitPucks[puck.index]
        , player = world.players[playerID]
        , paddle = world.paddles.get(player.paddle);
      ...
      if( world.timeouts.laserTimeout ) {
        ...
        if( world.paddles.values[0].data.laser == 1 ) world.paddles.values[0].data.laser = 2;
        else if( world.paddles.values[1].data.laser == 1 ) world.paddles.values[1].data.laser = 2;
      }
      ...
      world.timeouts.laserTimeout = world.tick.setTimeout(function(paddle){
        ...
        dmaf.tell( "laser_over");
        paddle.data.laser = 2;
      }.bind(null,paddle), 5000) // TODO data.duration?
      break;

    case 'fog':
      dmaf.tell( "fog_activate");
...
exports.destroyExtra = function(world, extra){
  ...
  dmaf.tell( extra.data.id + "_removed");

  //map to correct dmaf-id, change this?
  switch(extra.data.id){
    case 'fog':
    case 'fireball':
    case 'multiball':
    case 'bulletproof':
    case 'laser':
      ...
      break;
    case 'ghostball':
      dmaf.tell( "ghost_ball_removed");
      break;
    case 'extralife':
      dmaf.tell( "extra_life_removed");
      break;
    case 'mirroredcontrols':
      dmaf.tell( "mirrored_controls_removed");
      break;
    case 'deathball':
      dmaf.tell( "death_ball_removed");
      break;
    case 'paddleresize':
      dmaf.tell( "paddle_resize_removed");
      break;
    case 'timebomb':
      dmaf.tell( "time_bomb_removed");
      break;
    }
