...
exports.createExtra = function(world, data, x, y){
  ...
  if( data.id == "death ball") {
    ...
    this.emit('renderer','toggleDeathball',{active:true})
    world.tick.clearTimeout(extra.data.deathballTimeout);
    extra.data.deathballTimeout = world.tick.setTimeout(function(){
      this.emit('renderer','toggleDeathball',{active:false})
    }.bind(this),(data.duration || 5)*1000);
  }
...
exports.hitExtra = function(world, puck, extra){
  ...
  switch(id){
    ...
    case 'mirrored controls':
      ...
      world.tick.clearTimeout(extra.data.mirrorTimeout);
      extra.data.mirrorTimeout = world.tick.setTimeout(function(){
    ...
    case 'fog':
      ...
      world.tick.clearTimeout(extra.data.fogTimeout);
      extra.data.fogTimeout = world.tick.setTimeout(function(){
    ...
    case 'multiball':
      ...
      world.tick.setTimeout(function(){
        ...
        actions.emit('removeExtra',id);
      },200) // 200ms or 12 frames

      // push the new puck backwards
      // so n.previous = p.previous and n.current = p.previous-p.velocity
      //vec.sub(n.previous,puck.velocity,n.current)
      actions.puckSpeedXY(world, n, 0, 20)
      actions.puckSpeedXY(world, puck, 0, -20)

      /*n.previous[0] = n.current[0]
      n.previous[1] = n.current[1]

      puck.previous[0] = puck.current[0]
      puck.previous[1] = puck.current[1]
      */
      break;
  }
}
