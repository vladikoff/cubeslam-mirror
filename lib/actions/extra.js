  ...
  switch(extra.id){
    ...
    case 'fireball':
      ...
      if( player ){
        ...
        // TODO
        // 1. mark the paddle as "charged" (+ visuals on paddle)
        paddle.data.fireball = 1;

        // 2. when any puck hits a "charged" paddle it will catch "on fire" (+ visual on puck, - visual on paddle) and increase speed by 20%
        // 3. when an "on fire" puck hits anything it will be removed?
      } else {
        console.log('haha noob. you have to hit it with the paddle (or shield) first...')
        // TODO should it not be "hit" if no player paddle already hit the puck?
      }
