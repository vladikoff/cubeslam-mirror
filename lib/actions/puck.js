...
exports.puckCheckSpeedAll = function(world, p){
  for(var i=0; i < world.pucks.length; i++){
    actions.puckCheckSpeed(world, world.pucks.values[i]);
  }
}
...
exports.puckToggleGhostFlag = function(world,puckIndex,active){
  var puck = world.pucks.get(puckIndex);
  if( active ){
    console.log('puck GHOST ADD %s %s %s',world.name,world.frame,puck.index)
    ...
    console.log('puck GHOST DEL %s %s %s',world.name,world.frame,puck.index)
  }
}
