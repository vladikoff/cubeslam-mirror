...
exports.puckCreateCenter = function(world){
  ...
  var speed = settings.data.unitSpeed * world.level.puck.maxspeed;
...
exports.puckCheckSpeed = function(world, p){
  ...
  if( Math.abs(p.velocity[1]) < minY ){
    ...
    console.log("apply min y ",p.velocity[1],minY);
  ...
  // fun.
  var speed = vec.len(p.velocity) / settings.data.unitSpeed;
  var minSpeed = 1;
  var maxSpeed = world.level.puck.maxspeed;
  if( speed < minSpeed ){
    console.log('puck was below min speed %s now at a comfortable %s',speed,minSpeed)
    actions.puckSpeed(world,p,minSpeed);

  } else if( speed > maxSpeed ){
    console.log('puck was above max speed %s now at a comfortable %s',speed,maxSpeed)
