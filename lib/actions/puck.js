...
exports.slowDownPuck = function(world,puck){
  if( world.multiplayer && world.latency ){
    var speed = actions.getPuckSpeed(world,puck);
    var s = speed * settings.data.unitSpeed;

    // do complicated stuff if damped
    if( puck.damping !== 1 ){
      var v = vec.len(puck.velocity);
      console.log('slowing down damped puck!',v,puck.damping,puck.dampUntil,s)
      // how many frames will it have to damp?
      var i = 0
        , t = 0;
      while( v > puck.dampUntil ){
        i += 1;
        t += v;
        v *= puck.damping;
      }
      console.log('will have to damp %s frames before being back to %s resulting in a avg extra speed of %s',i,s,t/i||0);
      s += (t/i)||0;
    }


    var frames = world.latency/settings.data.timestep;
    var multiplier = Math.abs((s - frames)/s);
    speed *= Math.max(multiplier, 0.8); // cap the multiplier to avoid freakishly slow pucks
    actions.puckSpeed(world,puck,speed);
    console.log('slowing down puck (adjusted for a latency of %sms)',world.latency,speed)
  }
}
...
exports.puckToggleGhostball = function(world,puckIndex){
  var puck = world.pucks.get(puckIndex);
...
exports.puckTimebombExplode = function(world,puckIndex,radius){
  var puck = world.pucks.get(puckIndex);
