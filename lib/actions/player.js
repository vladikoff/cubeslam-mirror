...
exports.playerHit = function(world,player,puck){
  ...
  if( !world.multiplayer || world.name == 'sync' ){
    if( player == world.opponent ) {
      actions.emit('renderer','hitOpponent',{point: puck.current[0]/aw})
    }
    else if( player == world.me ) {
      actions.emit('renderer','hitMe',{point: puck.current[0]/aw*0.5 + 0.5})
    }
  }

  // only send HIT if it was me who was hit in multiplayer
  // otherwise send it everytime. (AI sucks at networking)
  if( !world.multiplayer || player == world.me ){
