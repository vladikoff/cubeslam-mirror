...
exports.createObstacle = function(world,id,x,y){
  ...
  switch(id){
    ...
    case 'block-rect-2x2':
      body = world.createBody(shape,x,y,BodyFlags.STATIC | BodyFlags.BOUNCE)
      break;
    case 'block-rect-2x2-destroy':
      body = world.createBody(shape,x,y,BodyFlags.STATIC | BodyFlags.BOUNCE | BodyFlags.DESTROY)
      break;
    default:
...
function getObstacleShape(id){
  switch(id){
    ...
    case 'block-rect-6x4':
      return shapes.rect(settings.data.unitSize*6,settings.data.unitSize*4);
    case 'block-rect-2x2':
    case 'block-rect-2x2-destroy':
      return shapes.rect(settings.data.unitSize*2,settings.data.unitSize*2);
    default:
      throw new Error('unsupported obstacle: '+id)
  }
}