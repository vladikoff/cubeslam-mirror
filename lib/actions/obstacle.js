...
exports.hitObstacle = function(world,obstacle,puck){
  ...
  if( world.multiplayer && world.name == 'game' ){
    console.log('puck bounced on obstable replay!',world.frame)
    actions.emit('replay');
  }
...
function getObstacleShape(obstacle){
  switch(obstacle.id || obstacle){
    ...
    case 'diamond':
      return shapes.diamond(settings.data.unitSize*(obstacle.size||4))
    case 'hexagon':
      return shapes.hex(settings.data.unitSize*(obstacle.size||3))
    case 'octagon':
      return shapes.oct(settings.data.unitSize*(obstacle.size||8))
    case 'block-breakout':
