...
exports.hitShield = function(world,shield,puck){
  ...
  if( puck.id !== 'puck' ){
    throw new Error('invalid puck: '+puck.id)
    return
  }

  console.log("hit shield:" + world.frame)
 ...
 /* if( world.pucks.values.length > 1 ){
  ...
  //reset speed, why is this not working?
  /*world.puckBounces[puck.index] = 1;
...
exports.destroyShield = function(world,shield,puck){
  debug('%s destroy',world.name ,shield,puck)
