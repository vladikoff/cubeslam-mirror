...
exports.hitShield = function(world,shield,body){
  ...
  if( body.id == 'puck' ){
    ...
    if( world.multiplayer && world.name == 'game' ){
      // emit 'replay'. to trigger a replay after
      // a MISS()
      if( player == world.opponent ){
        console.log('replay because of opponent shield hit',world.frame)
        actions.emit('replay');

      } else {
        var paddle = world.paddles.get(player.paddle);
        inputs.record(inputs.types.EMIT,'miss',paddle.current[0])
