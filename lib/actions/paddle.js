...
exports.hitPaddle = function(world,puck,paddle){
  ...
  if( paddle.index === world.me.paddle ){
    ...
    if( world.multiplayer && world.name == 'game' ){
      inputs.record(inputs.types.EMIT,'hit',paddle.current[0])

      // slow down puck
      var p = world.pucks.values[0];
      p.data.originalSpeed = speed;
      speed *= .85
      actions.puckSpeed(world,p,speed);
      console.log('slowing down puck',speed)
    }
  } else {
    actions.emit('opponentPaddleHit');
    dmaf.tell('opponent_paddle_hit');
    console.log('%s %s opponent paddle hit at %s',world.name,world.frame,paddle.current[0])
  }
...
exports.resizePaddle = function(world,paddleIndex,scale){
  ...
  paddle.radiusSq = poly.radiusSq(paddle.shape,paddle.current)
  mat.free(m);
