...
CameraController.prototype = {
  ...
  setState : function( state ) {
    ...
    if( state == "lobby" ) {
      ...
      this.cameraUpdateFunc = function(){
        ...
        this.moveCamera(this.cameraPaddleSetupPosition);
      ...
      this.cameraUpdateFunc = function(){
       ...
       // if( this.activeUser )
       //   this.cameraOffset.x = 20*(this.activeUser.paddle.position.x/settings.data.arenaWidth);
  ...
  moveCamera : function(pointData){
    ...
    var transformedData = { origin: pointData.origin.clone(), target: pointData.target.clone()};
   
    var originSumX = transformedData.origin.x+this.cameraOffset.x
    var originSumY = transformedData.origin.y+this.aspectCompensationZ*2+this.cameraOffset.y
    var originSumZ = transformedData.origin.z+this.aspectCompensationZ+this.cameraOffset.z
    ...
    originCurrent.x += (originSumX-originCurrent.x)/4;
    originCurrent.y += (originSumY-originCurrent.y)/14;
    originCurrent.z += (originSumZ-originCurrent.z)/14;

    var targetSumX = transformedData.target.x+this.cameraOffset.x*2;
    var targetSumY = transformedData.target.y
    var targetSumZ = transformedData.target.z
    ...
    targetCurrent.x += (targetSumX-targetCurrent.x)/4;
    targetCurrent.y += (targetSumY-targetCurrent.y)/24;
    targetCurrent.z += (targetSumZ-targetCurrent.z)/24;

    this.camera.lookAt(targetCurrent);

    return originCurrent.distanceTo(transformedData.origin);
  }
