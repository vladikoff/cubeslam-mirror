...
function createBear( renderer, terrain ) {
    ...
    morphMesh.position.set(-settings.data.arenaWidth*.5-100,-40,600)
    morphMesh.rotation.y = Math.PI*0.5;

    var shadowPlaneGeo = new THREE.PlaneGeometry( 150,150,1,1);
    var shadowMesh = new THREE.Mesh( shadowPlaneGeo, Materials.terrainShadow );
    
    shadowMesh.rotation.x = -Math.PI*0.5;
    shadowMesh.position.set(morphMesh.position.x,-15,600);
    shadowMesh.rotation.y = 15*Math.PI/180
