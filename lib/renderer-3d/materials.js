...
function Materials(){
 ...
 // this.arenaSideFaces = new THREE.MeshFaceMaterial();
  ...
  this.playerACube = createCubeMaterial(this.arenaBorder,this.opponent)
...
Materials.prototype.update = function(world){
  ...
  //this.cpu.uniforms.time.value += 0.01;
...
function createArenaGrid(renderer){
  ...
  gridTexture.mapping = THREE.Vector2Mapping;
...
function createOverlayMaterial() {
  ...
  tapeTexture.mapping = THREE.Vector2Mapping;
...
function createShield() {
  var semiTrans1 = new THREE.MeshBasicMaterial({color:0xffffff,transparent:true, opacity:0.2,side:THREE.DoubleSide});

  var decalTexture = THREE.ImageUtils.loadTexture( "images/shield_decal.png" );
  decalTexture.minFilter = decalTexture.magFilter = THREE.LinearMipMapLinearFilter;
