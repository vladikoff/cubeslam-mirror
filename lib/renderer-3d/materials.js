...
function Materials(renderer){
  ...
  this.arenaBorder = new THREE.MeshPhongMaterial({color:0xe5e4c6})
  ...
  this.localVideo = createTexture('videoInput')
  this.emptyVideo = new THREE.MeshLambertMaterial({color:0x000000})
  this.cpu = createCPUMaterial();
   
  this.paddleSetupBox = createPaddleSetupBox(this.localVideo);  
...
Materials.prototype.update = function(world){
  ...
  tex = this.localVideo.map;
  if( tex && tex.image.readyState === tex.image.HAVE_ENOUGH_DATA ) {
    tex.needsUpdate = true;
  }

  tex = this.paddleSetupBox[4].map;
  if( tex && tex.image.readyState === tex.image.HAVE_ENOUGH_DATA ) {
    tex.needsUpdate = true;
  }
...
function createCubeMaterial(){
  var front
    ...
    , front = new THREE.MeshPhongMaterial({color:0x000000}); 
...
function createPaddleSetupBox( localVideoMaterial){
  var side = new THREE.MeshPhongMaterial({color:0x0e3d74});

  var texture = new THREE.Texture(document.getElementById("videoInput"));
  //texture.offset = new THREE.Vector2( 0, -.2 )
  //texture.repeat  = new THREE.Vector2( 0, 0 )
  texture.generateMipmaps = false;

  var camMat = new THREE.MeshLambertMaterial({map:texture})
...
function createAnimalMaterial(){
  ...
  /*return new THREE.ShaderMaterial({
      ...
      uniforms:  {
          tDiffuse: {type:"t", value: animalTexture },
          topColor:    { type: "c", value: new THREE.Color( 0xffffff ) },
          bottomColor: { type: "c", value: new THREE.Color( 0x076fc8 ) },
...
*/
  ...
  return new THREE.MeshPhongMaterial({ 
    map:animalTexture,
    color:0x076fc8,ambient: 0x666666, specular: 0xffffff, shininess: 10,shading: THREE.FlatShading
