...
function Materials(){
  ...
  this.arenaSideColorMaterial = new THREE.MeshLambertMaterial({color:settings.level.arenaColor});
  ...
  this.reflectionBox = new THREE.MeshBasicMaterial({color:settings.level.arenaColor, side:THREE.DoubleSide})
  ...
  this.treeBranches = new THREE.MeshLambertMaterial({color:settings.level.treeBranchColor,shading: THREE.SmoothShading});
  //this.treeTrunk = new THREE.MeshLambertMaterial({color:settings.level.treeTrunkColor,shading: THREE.FlatShading})
  this.terrain1 = new THREE.MeshLambertMaterial( {color:settings.level.terrainColor1, shading:THREE.FlatShading });
  this.terrain2 = new THREE.MeshLambertMaterial( {color:settings.level.terrainColor2, shading:THREE.FlatShading });
  this.terrain3 = new THREE.MeshLambertMaterial( {color:settings.level.terrainColor3, shading:THREE.FlatShading });
  ...
  this.puck = new THREE.MeshLambertMaterial( { color: settings.level.puckColor })

  this.updateColors = function(){

    this.reflectionBox.color.setHex( settings.level.arenaColor );

    this.arenaTransMaterial.color.setHex( settings.level.arenaColor );

    var sideColor = new THREE.Color(settings.level.arenaColor).getHSV();
    sideColor.v *= 1.6;
    this.arenaSideColorMaterial.color = new THREE.Color().setHSV(sideColor.h,sideColor.s,sideColor.v);
    this.terrain1.color.setHex( settings.level.terrainColor1 );
    this.terrain2.color.setHex( settings.level.terrainColor2 );
    this.terrain3.color.setHex( settings.level.terrainColor3 );
    this.treeBranches.color.setHex( settings.level.treeBranchColor );
    this.animal.uniforms.diffuse.value.setHex( settings.level.terrainColor1 );
    //this.treeTrunk.color.setHex( settings.level.treeTrunkColor );
    updateGradientPlane( this.gradientPlane.map.image )
    this.gradientPlane.map.needsUpdate = true; 

  }

  settings.emitter.on("colorsUpdated", this.updateColors.bind(this) )
...
function createArenaTransMaterial( renderer ){
  return new THREE.MeshLambertMaterial({
    color: settings.level.arenaColor,
    opacity:0.8,
    transparent:true,
    depthWrite:false
  })
...
function updateGradientPlane( canvas ) {
  ...
  var r = dat.color.math.component_from_hex(settings.level.arenaColor,2);
  var g = dat.color.math.component_from_hex(settings.level.arenaColor,1);
  var b = dat.color.math.component_from_hex(settings.level.arenaColor,0); 
