...
function Materials(renderer){
  ...
  this.paddleSetupBox = createPaddleSetupBox(this.localVideoTexture,this.globalTimeUniform ); 
...
Materials.prototype.update = function(world){
  ...
  this.paddleSetupBox[4].uniforms.time.value = this.globalTimeUniform.value
...
function createPaddleSetupBox( localVideoTexture, globalUniformTime){
  var side = new THREE.MeshLambertMaterial({color:0x0e3d74});

  var camMat = new THREE.ShaderMaterial({
    uniforms:  {
      time: { type: "f", value:globalUniformTime},
