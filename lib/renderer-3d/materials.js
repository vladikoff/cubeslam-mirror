...
function Materials(){
 ...
 // this.arenaSideFaces = new THREE.MeshFaceMaterial();
  ...
  this.shield = new THREE.MeshLambertMaterial( {color:new THREE.Color(settings.data.shieldColor),depthWrite:false,transparent:true, opacity:0.2,side:THREE.DoubleSide} );
  ...
  this.clouds = new THREE.MeshLambertMaterial({wireframe:false,shading:THREE.FlatShading,color:0xedecd6});
  ...
  this.puck = new THREE.MeshLambertMaterial( { color: 0xedecd6 });
  ...
  this.bomb = new THREE.MeshLambertMaterial( {color:new THREE.Color(settings.data.fireColor), transparent:true, opacity:0.5,depthWrite:false});
  ...
  this.updateColors = function(){
    ...
    //this.arenaTransMaterial.color.set( settings.theme.arenaColor );
...
function createGradientPlane(){
  ...
  var shadowMaterial = new THREE.MeshBasicMaterial( { map: shadowTexture, depthWrite:false,transparent:false,side:THREE.DoubleSide} );
...
function updateGradientPlane( canvas ) {
  ...
  var c = new THREE.Color(settings.theme.arenaColor);

  gradient.addColorStop( 0, 'rgba('+c.r*255+','+c.g*255+','+c.b*255+',0)' );
  gradient.addColorStop( 0.3, 'rgba('+c.r*255+','+c.g*255+','+c.b*255+',1)' );
...
function createOpponentMaterial() {
  ...
  //document.getElementById("settingsGUIContainer").appendChild(canvas)
...
function createAnimalMaterial(){
  ...
  var uniforms = {
    ...
    details:  { type: "c", value: new THREE.Color(0xedecd6) },
...
function createFog() {
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: [
      ...
      "void main() {",
        "gl_FragColor = vec4( vec3 ( 0.8 ), 0.15*amount );",
      "}"
...
function createObstacleMaterial(renderer) {
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms: {
      tGrid: { type: "t", value: gridTexture},
      gridBrightness: { type: "f", value: settings.theme.gridBrightness },
      centerLineAmount: { type: "f", value: 1 },
...
function createForceMaterial() {
  var texture = THREE.ImageUtils.loadTexture( "images/texture_force.png" );
  texture.minFilter = THREE.LinearFilter;
  texture.magFilter = THREE.LinearFilter;
