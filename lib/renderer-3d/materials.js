...
function Materials(){
  ...
  var texture = THREE.ImageUtils.loadTexture( "images/grid_trans.png" );
  texture.generateMipmaps = true;
  texture.minFilter = THREE.LinearFilter;
  texture.magFilter = THREE.LinearFilter;

  texture.repeat = new THREE.Vector2(0.099*0.21,0.178*0.3);
  texture.offset = new THREE.Vector2(0,0.902);
...
*/
    ...
    updateIdleReflection(this.idleReflection.map.image)
  ...
  function tweenColor(target, to, updateCallback ) {
    ...
    if( hDiff > 0.6 ) {
      ...
      TweenMax.to(fromHSV,0.25, {h: edge, onComplete:function(){
        ...
        TweenMax.to(fromHSV,0+.25, {h: toHSV.h});
      }.bind(this)}) 
    }
    else {
      TweenMax.to(fromHSV,.5, {h: toHSV.h}) 
    }

    TweenMax.to(fromHSV,.5, { s: toHSV.s, v: toHSV.v, onUpdate:function(){
      target.setHSV(fromHSV.h, fromHSV.s, fromHSV.v); 
      if( updateCallback) updateCallback(fromHSV)
    }.bind(this)}) 
  }
...
function updateIdleReflection( canvas ) {
  var context = canvas.getContext( '2d' );
  var gradient = context.createLinearGradient( 0, 0, 0 , canvas.height);
  var c = new THREE.Color(0xedecd6);
  var c2 = new THREE.Color(settings.theme.arenaColor);
...
function createFog() {
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader: [
      ...
      "void main() {",
        "gl_FragColor = vec4( vec3 ( 0.8 ), 0.25*amount );",
      "}"
