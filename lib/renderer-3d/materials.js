...
function createCPUMaterial() {
  ...
  canvas.width = 512;
  canvas.height = 512;
  var brokenCtx = canvas.getContext("2d");

  brokenTexture = new THREE.Texture( canvas );
  ...
  /*return new THREE.ShaderMaterial({
    ...
    return cpuMaterial;
    //new THREE.MeshLambertMaterial({color:0xe5e4c6,map:texture, side:THREE.DoubleSide})
}
