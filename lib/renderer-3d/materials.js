...
function Materials(){
  ...
  this.updateColors = function(){
    ...
    this.icon.uniforms.diffuse.value.setHex( settings.level.theme.iconColor );
    this.iconReflection.uniforms.diffuse.value.setHex( settings.level.theme.iconColor );
    this.animal.uniforms.ambient.value.setHex( settings.level.theme.terrainColor1 );
  ...
  return new THREE.ShaderMaterial({
    ...
    uniforms:  THREE.UniformsUtils.merge( [
...

      {
        ...
        "diffuse": { type: "c", value: new THREE.Color(settings.level.theme.iconColor) },
...
//  context.fillStyle = "#000000";
 ...
 // context.globalCompositeMode = 'destination-in';
  ...
  //texture.anisotropy = .maxAnisorenderer.renderer.getMaxAnisotropy();
  texture.minFilter = texture.magFilter = THREE.LinearMipMapLinearFilter;
...
*/
    ...
    fragmentShader: [
      ...
      "void main() {",
        "gl_FragColor = vec4( vec3 ( 0.8 ), 0.16*amount );",
      "}"
