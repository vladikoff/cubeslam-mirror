...
function Materials(){
  ...
  var miniSceneRenderTarget = new THREE.WebGLRenderTarget( 64, 64,  { minFilter: THREE.LinearLinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat, useStencilBuffer:false } );
  miniSceneRenderTarget.generateMipmaps = false;
  this.overlayNoise = new THREE.MeshBasicMaterial({map:miniSceneRenderTarget});
 ...
 // this.arenaSideFaces = new THREE.MeshFaceMaterial();
  ...
  this.updateColors = function(){
    ...
    this.arenaSideColorMaterial.color.set(settings.theme.arenaColor);//color = new THREE.Color().setHSV(sideColor.h,sideColor.s,sideColor.v);
