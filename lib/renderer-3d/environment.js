...
function Environment(renderer){
  ...
  //this.animals = createAnimals(renderer)
...
Environment.prototype = {
  ...
  update: function(world,alpha){
    ...
    for(var i=this.extras.length; i < world.extras.length; i++ )
      this.extras.push(createExtra(this.arena, world, this.icons, world.extras[i]));
...
function createPaddleSetup(renderer,terrain){
  ...
  var geo = new THREE.CubeGeometry(900,675,50,1,1,1,renderer.materials.paddleSetupBox);
...
function createPaddleSetupSymbol(renderer, paddleSetupBox){
  ...
  var symbolsMesh = new THREE.Mesh(symbolsGeo, renderer.materials.scanSymbols )
  ...
  labelMesh = new THREE.Mesh(decalGeo, renderer.materials.paddleSetupDecal )
...
function createTerrain(renderer){
  ...
  var noise = null//new SimplexNoise()
    ...
    , h = 1000
    , n = 0
    , arenaW = settings.data.arenaWidth+settings.data.puckRadius*2
    , arenaH = settings.data.arenaHeight+settings.data.puckRadius*2;
...
*/
  ...
  //distant terrain
  terrainMesh = createTerrainMesh(6000,2000,2505,20,20,new THREE.Color( 0x1f84d5),4,false)
  ...
  terrainMesh = createTerrainMesh(4000,3000,8505,20,5,new THREE.Color( 0x195475 ),4,false)
...
function createClouds(terrain) {
  ...
  //var geometry = new THREE.SphereGeometry(300,8,8);
  //var geometry = THREE.GeodesicSphereGeometry(3,300);
  ...
  var cloudMesh = new THREE.Mesh(mergeGeometry, new THREE.MeshLambertMaterial({wireframe:false,shading:THREE.FlatShading,color:0xffffff}) )
...
function createPuck(arena){
  debug('create puck')
  var puckGeo = new THREE.CubeGeometry( 100,100,100 ) // size will be set in update instead...
...
function createExtra(arena, world, icons, extra){
  ...
  // hack to figure out which id the extra has
  // TODO don't hack it
  var icon;
  if( extra === extras.speedball )
    icon = 'speedball'
  if( extra === extras.extraball )
    icon = 'extraball'

  // extra has an icon
  if( icon && icons[icon] ){
    var icon = icons[extra.icon];
    icon.position.x = extra.current[0];
    icon.position.z = extra.current[1];
    arena.add(icon);
    return icon;
  ...
  // extra is a polygon
    ...
    shapeMesh.position.x = extra.current[0];
    shapeMesh.position.z = extra.current[1];
