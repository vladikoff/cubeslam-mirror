...
Environment.prototype = {
  ...
  update: function(world,alpha){
    ...
    while(world.remove.length){
      ...
      if( (i=this.bullets.indexOf(obj)) !== -1 ){
        removeBullet(obj);
        this.bulletPool.returnObject(obj.poolId);
        // dont remove from the bullets array to avoid
        // overwriting them, just set to null so we can
        // quickly skip them.
        // this.bullets.splice(i,1);
        this.bullets[i] = null;
      }

      else if( (i=this.pucks.indexOf(obj)) !== -1 ){
        removePuck(obj);
    ...
    // create bullets
    for(var i=this.bullets.length; i < world.bullets.length; i++ ){
      var body = world.bullets[i];
    ...
    for(var i=0; i < world.bullets.length; i++){
      ...
      if( world.bullets[i] == null) continue;

      var shot = world.bullets[i]
        , mesh = this.bullets[i];
      if( !mesh ) continue;
