...
Environment.prototype = {
  ...
  update: function(world,alpha){
    ...
    // pucks create (TODO pool this, or at least pre-instantiate)

    // pucks update

    // forces create (TODO pool this, or at least pre-instantiate)

    // extras (obstacles) forces (TODO pool this, or at least pre-instantiate)
...
function createArena(renderer){
  ...
  var transArenaMaterial = new THREE.MeshPhongMaterial({
  ...
  var bottomGeo = new THREE.PlaneGeometry(w,d,1,1)
    , bottomMesh = new THREE.Mesh( arenaBottomGeo, transArenaMaterial );
...
function createTerrain(renderer){
  var geometry = new THREE.GeometryLoader().parse(renderer.geometry.terrain1)
  ...
  renderer.container.add(terrainMesh);
...
function createForrest(renderer) {
  ...
  for ( var i = 0; i < 70; i ++ ) {
    ...
    renderer.container.add(trunkMesh);
  }
}



function createForce(renderer, world, force){
  ...
  forceMesh.position.y = settings.data.arenaSurfaceY;
  renderer.container.add(forceMesh);
  return forceMesh;
}
...
function createPuck(){
  ...
  puckMesh.position.y = settings.data.arenaSurfaceY;
  renderer.container.add(puckMesh);
  return puckMesh;
}
...
function createExtra(world, extra){
