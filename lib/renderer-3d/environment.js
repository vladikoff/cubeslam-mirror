...
function createPaddleSetup(renderer,terrain){
  ...
  var geo = new THREE.CubeGeometry(900,700,50,1,1,1,renderer.materials.paddleSetupBox);
  ...
  foundationMesh.position.set(-2000,-100,470)
...
function createLights(terrain){
  ...
  var pointLight = new THREE.PointLight( 0xFFFFFF,1.6,2000 );
...
*/
    ...
    //if( i > 0 && i < 100) point.y = 600;

    //set z-points to edge
    if( i == 28 || i == 27) {
      point.z = settings.data.arenaHeight*.5 + settings.data.videoBoxDepth;
    }
    else if(i==51 || i == 53) {
      point.z = -settings.data.arenaHeight*.5 - settings.data.videoBoxDepth;
    }
  ...
  //distant terrain
  terrainMesh = createTerrainMesh(6000,2000,2205,20,20,new THREE.Color( 0x3968cc ),1.4,false)
  ...
  terrainMesh = createTerrainMesh(4000,3000,8205,10,10,new THREE.Color( 0x2548c9 ),1,false)
...
function createForrest(renderer,terrain) {
  ...
  shapeGeo.applyMatrix( new THREE.Matrix4().translate( new THREE.Vector3(0, 150, 0)));
  ...
  for (var px = gridX - 1; px >= 0; px--) {
    for (var py = gridY - 1; py >= 0; py--) {
      ...
      if( uvX > -0.2 && uvX < 0.2 && uvY > -0.5 && uvY < 0.5) continue;

      var n = noise.noise((uvX+0.5)*4, (uvY+0.5)*4 );

      if( n > 0.5) {
          
        var scale = (n*n)*1.5+Math.random()*0.2;
...
function createAnimals(renderer) {
  ...
  mooseMesh.position.set(-2000,-100,1000);
  mooseMesh.rotation.y=Math.PI;
  mooseMesh.scale.set(5,5,5)
