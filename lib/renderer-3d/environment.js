  ...
  for (var px = gridX - 1; px >= 0; px--) {
    for (var py = gridY - 1; py >= 0; py--) {
      ...
      if( n >= 0.55) {
        ...
        shadowMesh.rotation.set(intersectionNormal.x,intersectionNormal.y, intersectionNormal.z).normalize().negate();
...
//console.log(Geometry.clouds2);
  ...
  var cloudMesh;
   ...
   cloudMesh = new THREE.Mesh( Geometry.cloud ,Materials.clouds);
  ...
  cloudMesh = new THREE.Mesh( Geometry.cloud ,Materials.clouds);
   ...
   cloudMesh.scale.set(15,20,20);
   terrain.add(cloudMesh);


   cloudMesh = new THREE.Mesh( Geometry.cloud ,Materials.clouds);
   cloudMesh.position.set(1000,1600,-10000);
   cloudMesh.scale.set(30,30,17+Math.random()*2);
   terrain.add(cloudMesh);
   ...
   cloudMesh.scale.set(20,20,27+Math.random()*2);
   terrain.add(cloudMesh);
 ...
 /* //set 2
  ...
  for (var i = 3; i >= 0; i--) {
     cloudMesh = new THREE.Mesh( Geometry.cloud ,Materials.clouds);
     cloudMesh.position = pos[i];
     cloudMesh.scale.set(6,5+Math.random()*2,5+Math.random()*2);
     terrain.add(cloudMesh);
  };*/
...
function removeShield(shield, fast ){
  ...
  if( shield.parent ) {
    ...
    TweenMax.to(shieldRef.scale,0.2,{delay:animDelay,y:0.01, ease:Sine.easeIn, onComplete:function(){
...
function createShieldPool() {
  ...
  var geom = new THREE.BoxGeometry(100,settings.data.arenaSideHeight*2,100,1,1,1,{ px: true, nx: true, py: true, ny: false, pz: true, nz: false })
