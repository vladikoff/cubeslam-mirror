    ...
    this.onadded = function(type,world,body){
      ...
      switch(type){
        case 'extra':
          ...
          break;
        case 'bullet':
          var obj = Bullet.alloc().add(this.arena, body, world);
        ...
        case 'puck':
          ...
          break;
        case 'shield':
          var obj = Shield.alloc().add(this.arena, body, world);
  ...
  update: function(world,alpha){
    ...
    for(var i=0; i < world.pucks.values.length; i++){
      ...
      if( this.pucks.has(puck.index) ){
        var mesh = this.pucks.get(puck.index);
        mesh.position.x = ((puck.current[0]+puck.offset[0])*alpha + (puck.previous[0]+puck.offset[0])*(1-alpha)) - hw;
        mesh.position.z = ((puck.current[1]+puck.offset[1])*alpha + (puck.previous[1]+puck.offset[1])*(1-alpha)) - hh;

      } else if( !world.multiplayer ){
        console.error('renderer missing a puck of index %s',puck.index);
    ...
    /*for(var i=0; i < world.bullets.values.length; i++){
      ...
      if( this.bullets.has(shot.index) ){
        var mesh = this.bullets.get(shot.index);
        mesh.position.x = shot.current[0] - hw;
        mesh.position.z = shot.current[1] - hh;
      } else if( !world.multiplayer ){
        console.error('renderer missing a bullet of index %s',shot.index);
    ...
    for(var i=0; i < world.bullets.values.length; i++){
      ...
      if( this.bullets.has(bullet.index) ){
        this.bullets.get(bullet.index).update(world);
      }
    }
    ...
    for(var i=0; i < world.forces.values.length; i++){
      ...
      if( this.forces.has(force.index) ){
        ...
        if( mesh.visible != force.active ){
          mesh.material = force.type == "repell"
              ? Materials.forceRepell
              : Materials.forceAttract
        }
    ...
    // update shields
    for(var i=0; i < world.shields.values.length; i++){
...
function createArena(renderer){
  ...
  tempMesh.rotation.x = Math.PI*-.5
  tempMesh.position.set(0,0,-d*.5-boxDepth*.5)
  THREE.GeometryUtils.merge(finalGeo, tempMesh);
  ...
  centerLineMeshFloor.rotation.z = Math.PI*.5;
  centerLineMeshFloor.rotation.x = Math.PI*.5;
  ...
  digitPlane.position.z = settings.data.arenaHeight*.5 - settings.data.unitSize*2 -  9*settings.data.unitSize*0.5;
  digitPlane.position.x = settings.data.arenaWidth*.5 - settings.data.unitSize*2 - 5*settings.data.unitSize*0.5-5;
...
function createExtra(arena, world, icons, extra){
  ...
  if( icon && icons[icon] ){
    ...
    var icon = getExtraMesh(arena,icons,icon);
...
function createBulletPool() {
  ...
  BulletPool.createObject = function(){
    var bulletMat = Materials.bullet
