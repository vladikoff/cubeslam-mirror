...
function createExtra(arena, world, icons, extra){
  ...
  // extra is a polygon
    ...
    var shapeGeo = new THREE.ExtrudeGeometry( shape, {
      ...
      , shapeMesh = new THREE.Mesh( shapeGeo , new THREE.MeshFaceMaterial([Materials.obstacleSide, Materials.obstacle2]) );
...
function createShield(arena, body){
  ...
  TweenMax.to( mesh.scale, 0.5, {y:1,delay:0.2,ease:Sine.easeOut});
...
function removeShield(shield){
  debug('remove shield')
  if( shield.parent ) {
    TweenMax.to(shield.scale,0.2,{y:0.01, ease:Sine.easeIn, onComplete:function(){
...
function createShieldPool() {
  ...
  ShieldPool.createObject = function(){
    ...
    var mesh = new THREE.Mesh( geom, Materials.shield ); 
    mesh.name = 'shield';
    return mesh;
  }.bind(this)
