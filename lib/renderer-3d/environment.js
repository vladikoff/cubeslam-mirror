...
function createArena(renderer){
  ...
  var reflectionBoxGeo = new THREE.CubeGeometry(w,200,d+boxDepth*2,1,1,1,null, { px: true, nx: true, py: false, ny: true, pz: true, nz: true });
...
function createLights(terrain){
  ...
  var hemLight = new THREE.HemisphereLight(0x076fc8, 0xeeeeee,0.6);
...
function createForrest(renderer,terrain) {
  ...
  for (var px = gridX - 1; px >= 0; px--) {
    for (var py = gridY - 1; py >= 0; py--) {
      ...
      if( uvX > -0.2 && uvX < 0.2 && uvY > -0.4 && uvY < 0.4) continue;
      ...
      if( n > 0.5) {
        ...
        trunkMesh.position.set(
          uvX*6000 +Math.random()*100,
          -100,
          uvY*6000+Math.random()*100
        )
