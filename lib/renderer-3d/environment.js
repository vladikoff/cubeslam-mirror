...
Environment.prototype = {
  ...
  update: function(world,alpha){
    ...
    for(var i=0; i < world.pucks.values.length; i++){
      ...
      if( mesh ){
        mesh.position.x = puck.current[0] + (puck.current[0]-puck.previous[0])*alpha - hw + puck.offset[0];
        mesh.position.z = puck.current[1] + (puck.current[1]-puck.previous[1])*alpha - hh + puck.offset[1];

        mesh.scale.x = (puck.aabb[1] - puck.aabb[3])/100
        mesh.scale.z = (puck.aabb[2] - puck.aabb[0])/100
      } else {
        // the puck hasn't been created in "sync" yet.
      }
    }
