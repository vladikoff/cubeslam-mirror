...
Environment.prototype = {
  ...
  update: function(world,alpha){
    ...
    for(var i=0; i < world.pucks.values.length; i++){
      ...
      mesh.position.x = puck.current[0] - hw;
      mesh.position.z = puck.current[1] - hh;
...
function createTerrain(renderer){
  ...
  terrainMesh.position.y = settings.data.arenaSurfaceY;
  ...
  terrainMesh.position.x = Math.random()*5000-2500;
  terrainMesh.position.y = settings.data.arenaSurfaceY - 100;
  terrainMesh.scale.x = 4;
...
//console.log(Geometry.clouds2);
  var cloudMesh;
  var pos = [
    new THREE.Vector3(400,1000,-3000),
    new THREE.Vector3(-4000,1000,-3500),
    new THREE.Vector3(4000,1000,-3000),
    new THREE.Vector3(2000,1000,-5000),
  ]
...
function createPuck(arena, puck){
  ...
  puckMesh.scale.y = ((puck.aabb[1] - puck.aabb[3])/100)*2
...
function createExtra(arena, world, icons, extra){
  ...
  // extra is a polygon
    ...
    var shapeGeo = new THREE.ExtrudeGeometry( shape, {
      ...
      , shapeMesh = new THREE.Mesh( shapeGeo, Materials.obstacle2  );
    ...
    new TWEEN.Tween(shapeMesh.scale)
      .to({z:1},400)
...
function removeObstacle(obstacle){
  ...
  new TWEEN.Tween(obstacle.scale)
    .to({z:0},400)
