...
Environment.prototype = {
  ...
  update: function(world,alpha){
    ...
    for(var i=0; i < world.pucks.length; i++){
      ...
      if( mesh.material.color.getHex() != settings.level.puckColor)
        mesh.material.color.setHex( settings.level.puckColor );
    }
...
function createForrest(renderer,terrain) {
  ...
  //temp meshes
  var trunkMesh = new THREE.Mesh( trunkGeo, Materials.treeTrunk );
  var shapeMesh = new THREE.Mesh( shapeGeo, Materials.treeBranches );
