...
function Environment(renderer){
  ...
  this.bulletPool = createBulletPool(new THREE.PlaneGeometry( settings.data.arenaWidth/settings.data.arenaColumns,150 ))
...
Environment.prototype = {
  ...
  reset: function(){
    ...
    while(this.bullets.values.length)
      removeExtra(this.bullets.values.pop());
    while(this.obstacles.values.length)
      removeExtra(this.obstacles.values.pop());
  ...
  update: function(world,alpha){
    ...
    while(world.remove.length){
      ...
      if( (obj=this.bullets.get(id)) ){
        removeBullet(obj,this.bulletPool);
        this.bullets.del(id)
      }

      else if( (obj=this.pucks.get(id)) ){
        removePuck(obj);
        this.pucks.del(id);
      }

      else if( (obj=this.forces.get(id)) ){
        removeForce(obj);
        this.forces.del(id);
      }

      else if( (obj=this.extras.get(id)) ){
        removeExtra(obj);
        this.extras.del(id);
      }

      else if( (obj=this.obstacles.get(id)) ){
        removeExtra(obj);
    ...
    // TODO the world.x.values length comparison is dumb. we need a better comparison
    //      thing so we can do a diff(this.pucks,createPuck) ?
    //      because this will fail as soon as the values array is unordered (which
    //      means as soon as it has a deleted item)

    // create pucks
    for(var i=this.pucks.values.length; i < world.pucks.values.length; i++ ){
      var body = world.pucks.values[i];
      var obj = createPuck(this.arena, body)
      this.pucks.set(body.id,obj)
    }

    // create forces
    for(var i=this.forces.values.length; i < world.forces.values.length; i++ ){
      var body = world.forces.values[i];
      var obj = createForce(this.arena, world, body)
      this.forces.set(body.id,obj)
    }

    // create extras
    for(var i=this.extras.values.length; i < world.extras.values.length; i++ ){
      var body = world.extras.values[i];
      var obj = createExtra(this.arena, world, this.icons, body)
      this.extras.set(body.id,obj)
    }

    // create bullets
    for(var i=this.bullets.values.length; i < world.bullets.values.length; i++ ){
      var body = world.bullets.values[i];
      var obj = createBullet(this.bulletPool, this.arena, world, body)
      this.bullets.set(body.id,obj)
    }

    // create obstacles
    for(var i=this.obstacles.values.length; i < world.obstacles.values.length; i++ ){
      var body = world.obstacles.values[i];
      var obj = createExtra(this.arena, world, this.icons, body)
      this.obstacles.set(body.id,obj)
    }

    // update pucks
    for(var i=0; i < world.pucks.values.length; i++){
      var puck = world.pucks.values[i]
        , mesh = this.pucks.get(puck.id);
...
function createIcons(renderer){
  ...
  for (var i = iconTypes.length - 1; i >= 0; i--) {
    ...
    iconGeo.applyMatrix( new THREE.Matrix4().makeScale(25,25,25) );
...
// draw obstacles with THREE.Shape()
// http://mrdoob.github.com/three.js/examples/webgl_geometry_shapes.html
function createBullet(bulletPool, arena, world, body){
  debug('create bullet')
  ...
  mesh.position.x = body.current[0]-hw;
  mesh.position.z = body.current[1]-hh;
...
function removeBullet(bullet,pool){
  debug('remove bullet')
  if( bullet && bullet.parent ){
    bullet.parent.remove(bullet);
    pool.returnObject(bullet.poolId);
  }
}

function createBulletPool( bulletGeo ) {
  ...
  BulletPool.createObject = function(){
    ...
    var r = settings.data.puckRadius
      , bulletMat = new THREE.MeshLambertMaterial( { transparent:true, depthWrite:false,color: 0xffff00})
