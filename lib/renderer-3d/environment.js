...
Environment.prototype = {
  ...
  update: function(world,alpha){
    ...
    for(var i=0; i < world.pucks.values.length; i++){
      ...
      mesh.position.x = puck.previous[0] + (puck.current[0]-puck.previous[0])*alpha - hw;
      mesh.position.z = puck.previous[1] + (puck.current[1]-puck.previous[1])*alpha - hh;

      //mesh.position.x = mesh.position.x + ((puck.current[0] - hw)-mesh.position.x)*alpha;
      //mesh.position.z = mesh.position.z + ((puck.current[1] - hh)-mesh.position.z)*alpha;
...
*/
  ...
  //distant terrain
  terrainMesh = createTerrainMesh(8000,2000,2505,20,3,new THREE.Color( 0x1f84d5),4,false, Materials.terrain2)
