...
function Environment(renderer){
  ...
  //test
  //createForce( this.arena, null, {type:"attract",position:[settings.data.unitSize*.5,settings.data.unitSize*.5]});
...
Environment.prototype = {
  ...
  setupAddedRemoved: function(worldName){
    // TODO if world.replaying we want to not act until
    //      the first render (or maybe a separate event?)
    //      so we would only create/remove the ones that
    //      has changed _after_ the replay. If a puck has
    //      been created/removed/created again we don't
    //      really need to remove it in the first place.
    //
    ...
    actions.on('replay end',function(){
      ...
      if( added.length || removed.length ){
        console.log('done with replay. %s bodies added. %s bodies removed',added.length,removed.length)
  ...
  showLevelEditorMarker : function(paramObj) {
    ...
    marker.scale.y = settings.data.unitSize*4/100
...
function createForce(arena, world, force){
  ...
  TweenMax.to(forceMesh,0.3,{repeat:-1,onRepeat:function(){

    //force attract
    var tilePos = Materials.forceAttract.map.tilePos;
    tilePos.x += 1;
    if( tilePos.x > 1) {
      tilePos.x = 0;
      tilePos.y++;
    ...
    Materials.forceAttract.map.offset.x = tilePos.x/2
    Materials.forceAttract.map.offset.y = tilePos.y/2

    //force repell
    tilePos = Materials.forceRepell.map.tilePos;
    tilePos.x -= 1;
    if( tilePos.x < 0) {
      tilePos.x = 1;
      tilePos.y--;
    ...
    Materials.forceRepell.map.offset.x = tilePos.x/2
    Materials.forceRepell.map.offset.y = tilePos.y/2
  }})
