...
function CPU(renderer) {
  ...
  this.settings = {
    ...
    characters: [
      {
        name: "Clem",
        initPos: new THREE.Vector3(0,-200,100),
        scale: 6
      }
    ],
...
CPU.prototype.loadCharacter = function( index ){
  ...
  body.setAnimationLabel("left",0,21);
...
CPU.prototype.update = function(time){
  ...
  if( !this.isAnimating ) {
   ...
   this.avatar.position.y = this.currentCharacterData.initPos.y + ImprovedNoise.noise(0,time,0)*this.settings.tremble;
    ...
    this.avatar.position.z = this.currentCharacterData.initPos.z+this.settings.concentrate*150;
...
CPU.prototype.setPaddleX = function( value ) {
  if( this.hand ) this.hand.position.x = value*.05-50;
}
