...
Player.prototype = {
  ...
  paddleToCenterUpdate: function(){
    this.paddle.position.x += (0-this.paddle.position.x)/5
  },
  ...
  update: function(alpha){
    ...
    if( settings.data.debugFireball && !this.fireSkin.isActive ) {
      this.fireSkin.attachToMesh(this.paddle)
    }
...
*/
    ...
    if( this.voxelsExploding.length > 0 ) {
      ...
      for (var i = this.voxelsExploding.length - 1; i >= 0; i--) {
        ...
        // floor condition
        if (voxel.position.y < -videoHeight*.5+voxel.radius + settings.data.arenaSideHeight + voxel.radius*voxel.bounceLevel + voxel.bounceLimit) {
...
function createIdlePlane(){
  ...
  var idlePlane = new THREE.Mesh( new THREE.PlaneGeometry(w-20,h+3,1,1), Materials.arenaBorder);
