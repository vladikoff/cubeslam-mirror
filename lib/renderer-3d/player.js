...
Player.prototype = {
  ...
  setCubePosition: function( mirrored  ) {
    ...
    if( mirrored ) {
      this.cube.position.z = -hd - settings.data.videoBoxDepth*0.5;
    ...
    for (var tz = 0; tz < this.voxelsGridSize.z; tz++) {
      for (var tx = 0; tx < this.voxelsGridSize.x; tx++) {
        for (var ty = 0; ty < this.voxelsGridSize.y; ty++) {
          ...
          voxel = new THREE.Mesh(voxelGeo, tz===0? Materials.playerBCube:Materials.playerBCube.materials[0] );
