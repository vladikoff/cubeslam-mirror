...
Player.prototype = {
  ...
  update: function(player,paddle){
    var r = settings.data.puckRadius
      , fw = settings.data.arenaWidth + r*2
      , fh = settings.data.arenaHeight + r*2
      , hw = fw/2
      , hh = fh/2;
    ...
    if (paddle) {
      ...
      this.paddle.position.z = (y*fh-hh)+settings.data.paddleDepth/2*(this.id ? 1 : -1);
...
function createShield(renderer,columns){
  ...
  var shieldGeo = new THREE.CubeGeometry(w,h,8,1,1,1,null, { px: false, nx: false, py: true, ny: false, pz: this.id==0?false:true, nz: this.id==0?true:false })
