...
Player.prototype = {
  ...
  heal : function(){
    ...
    .onComplete( function(){
        ...
        world.players[(scope.id == 0)? "b":"a"].hits.length = 0;
...
function createCube(renderer,id){
  ...
  if( id >= 1) {
    decalGeo.faceVertexUvs[0][0] = [
      new THREE.UV( 0.0,1 ),
      new THREE.UV( 0.0, -0.01 ),
      new THREE.UV( 1.0, -0.01 ),
      new THREE.UV( 1.0, 1 )
    ]
...
function createPaddle(renderer, id){
  debug('create paddle')
  var paddleGeo = new THREE.CubeGeometry( 100, settings.data.paddleDepth, settings.data.paddleDepth )
...
function createLazerPool( lazerGeo ) {
  ...
  lazerPool.createObject = function(){
    ...
    var r = settings.data.puckRadius
      , lazerMat = new THREE.MeshLambertMaterial( { transparent:true, depthWrite:false,color: settings.theme.puckColor})
