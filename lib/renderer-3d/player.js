...
Player.CubeMaterialCPU = "opponent"
Player.CubeMaterialNoise = "staticNoise"
Player.CubeMaterialRemote = "opponent"
Player.CubeMaterialLocal = "opponent"
...
Player.prototype = {
  ...
  setMaterial: function( newMaterial, hideCube ){
    ...
    if( hideCube ) {
      ...
      this.hide(true, function() {
         ...
         this.cube.material.materials[4] = Materials[newMaterial];

        //setup material if cpu or remoteVideo
        if( newMaterial == Player.CubeMaterialRemote || newMaterial == Player.CubeMaterialCPU ) {

          if( newMaterial == Player.CubeMaterialCPU ) {
            if( !this.cpu ) {
              this.cpu = new CPU(this.renderer.renderer);
          ...
          Materials.opponent.uniforms.tBroken.value = this.brokenTexture;
        }
        else {
          this.videoMaterial = null;
        }
  ...
  update: function(paddle,alpha){
    ...
    if (paddle) {
      ...
      this.paddle.position.z = y+this.paddle.offsetZ+(d/8*3)*(this.id ? 1 : -1);
    }

    if( this.cpu ){

      this.cpu.setPaddleX( this.paddle.position.x )
      this.time += 0.1;
  ...
  setShields : function( count ) {
    ...
    for(var i=0; i < count; i++) {
      ...
      shield.visible = true;
      shield.scale.x = (settings.data.arenaWidth/count - (count-1)*settings.data.shieldPadding)/100;
      shield.position.x = -settings.data.arenaWidth*.5 + i*settings.data.arenaWidth/count + i*settings.data.shieldPadding;

      TweenMax.to(shield.scale,0.3,{y:1,delay:i*0.1, ease:Back.easeOut});
    }
  },
...
function createCube(renderer,id){
  ...
  var cubeGeo = new THREE.CubeGeometry(w-20,h,settings.data.videoBoxDepth-10,1,1,1)

    , cube = new THREE.Mesh(cubeGeo, id==0?Materials.playerACube:Materials.playerBCube);
  ...
  setTimeout(function(){cube.material.materials[4] = Materials.arenaBorder},500);
...
function createPaddle(renderer, id){
  ...
  paddle.position.z = (hh-(settings.data.unitSize/2*3))*(id==0 ? 1 : -1);
...
function createShield() {
  ...
  var geo = new THREE.CubeGeometry(100,settings.data.arenaSideHeight,10,1,1,1, { px: true, nx: true, py: true, ny: false, pz: true, nz: false });
  geo.applyMatrix( new THREE.Matrix4().translate( new THREE.Vector3(50, settings.data.arenaSideHeight*.5, 0)));

  var shield = new THREE.Mesh( geo, Materials.shield );
  shield.visible = false;
  shield.scale.y = 0.01;

  /*var shieldReflection = new THREE.Mesh( geo, new THREE.MeshBasicMaterial({color:0xffffff}) );
  shieldReflection.scale.y = -1;
  shield.add(shieldReflection);
*/
  return shield;
}
