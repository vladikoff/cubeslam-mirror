...
Player.prototype = {
  ...
  reset: function(){
    ...
    this.paddle.position.z = (hd-(paddleDepth/4))*(host ? -1 : 1);
  ...
  update: function(player,paddle){
    ...
    if (paddle) {
      ...
      this.paddle.position.z = y+this.paddle.offsetZ+((paddleDepth/4))*(this.id ? 1 : -1);
      this.paddle.scale.z = paddleDepth/200;
    }
...
function createPaddle(renderer, id){
  debug('create paddle')
  var paddleGeo = new THREE.CubeGeometry( 100, 100, 100 )
