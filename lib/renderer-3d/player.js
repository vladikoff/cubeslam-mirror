...
function createShield(renderer,columns, host ){
  ...
  var shieldMat = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      tVideo: { type: "t", value: !host?renderer.materials.localVideoTexture:renderer.materials.remoteVideoTexture },
...
function createLazerPool( lazerGeo ) {
  ...
  lazerPool.createObject = function(){
    ...
    var r = settings.data.puckRadius
      , lazerMat = new THREE.MeshLambertMaterial( { transparent:true, depthWrite:false,color: settings.data.puckColor})
