...
function Player(renderer,id){
  ...
  this.screenDecal = createScreenDecal(this.cube);
...
Player.prototype = {
  ...
  setDecal: function( id, multiplayer ) {
    ...
    console.log("is multiplayer:" + multiplayer)
    this.screenDecal.material.map.offset.set( (id===0)?0:0.5, (multiplayer)?0:0.5 );
    this.screenDecal.material.map.uvsNeedUpdate = true;
  },
...
function createScreenDecal( cube ) {
  //decal
  var w = settings.data.arenaWidth
    , hw = w*.5
    , h = w/16*9
  ...
  var decalGeo = new THREE.PlaneGeometry(300,300,1,1);
  var decalMesh = new THREE.Mesh( decalGeo,Materials.screenDecal)
