...
Player.prototype = {
  ...
  update: function(paddle,alpha){
    ...
    if (paddle) {
      ...
      var paddleWidth = (paddle.aabb[1] - paddle.aabb[3])
      var parentScale = paddleWidth/100
      this.paddle.scale.x = parentScale;
      //this.paddle.scale.z = (paddle.aabb[2] - paddle.aabb[0])/100;

      if(this.paddle.children.length > 0 ){

        var scaleUp = outerWidth/100;
        this.paddle.children[0].scale.x = (1-parentScale-0.4) / parentScale ;
      }
