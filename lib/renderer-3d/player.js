...
Player.prototype = {
  ...
  show: function( toIdle ){
    ...
    TweenMax.to( this.cube.position, 3, {ease:Sine.easeOut,y: toIdle?-h*.5+settings.data.arenaSideHeight:h*.5 });
    ...
    for (var tz = 0; tz < this.voxelsGridSize.z; tz++) {
      for (var tx = 0; tx < this.voxelsGridSize.x; tx++) {
        for (var ty = 0; ty < this.voxelsGridSize.y; ty++) {
          ...
          var voxel = new THREE.Mesh(voxelGeo, tz==0? Materials.playerBCube:Materials.playerBCube.materials[0] );
  
          voxel.position.set( voxelSizeX*tx-w*.5 + voxelSizeX*.5, h*.5-voxelSizeY*ty - voxelSizeY*.5 ,d*.5 - tz*voxelSizeZ- voxelSizeZ*.5);
  ...
  explode: function( normalizedHitPosition ) {
    ...
    if( !this.mirrored ) normalizedHitPosition = 1-normalizedHitPosition;
  ...
  hit : function( normalizedImpactX, callback ){
    ...
    if( !this.mirrored ) normalizedImpactX = 1-normalizedImpactX;
