...
function LaserSkin() {
  ...
  var container = new THREE.Object3D();
  var geo = new THREE.BoxGeometry(100,100,100,1,1,1, { px: true, nx: true, py: true, ny: false, pz: true, nz: true });
  geo.applyMatrix( new THREE.Matrix4().translate( new THREE.Vector3(50,50, 0)));
...
LaserSkin.prototype.update = function(){
  ...
  this.container.scale.set(
    parentWidthWithMargins/100*this.progress +0.001,
    (bb.y+settings.data.unitSize*.5)/100,
    (bb.z+settings.data.unitSize)/100
  )
}
