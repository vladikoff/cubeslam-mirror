  ...
  update: function(world,alpha){
    ...
    if( world.paddles.length ){
      ...
      if( paddleBody.data.laser && !this.laserSkin1.isActive ){
        var paddleMesh = this.renderer.players.me.paddle;
        paddleBody.data.laser = toggleEffect(paddleMesh,this.laserSkin1,paddleBody.data.laser);
      }
      ...
      if( paddleBody.data.laser && !this.laserSkin2.isActive ){
        var paddleMesh = this.renderer.players.opponent.paddle;
        paddleBody.data.laser = toggleEffect(paddleMesh,this.laserSkin2,paddleBody.data.laser);
      }
    ...
    for (var i = world.shields.values.length - 1; i >= 0; i--) {
      ...
      // see puck update above for reasoning
      if( !this.env.shields.has(shieldBody.index) && world.multiplayer ){
        continue;
      }

      var shieldMesh = this.env.shields.get(shieldBody.index);

      if( !shieldMesh ){
        // if this is multiplayer it must be a bug in stash#get() ?
        debugger;
        shieldMesh = this.env.shields.get(shieldBody.index); // trying again?
      }
