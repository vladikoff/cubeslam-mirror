...
Effects.prototype = {
  ...
  update: function(world,alpha){
    ...
    for (var i = world.pucks.values.length - 1; i >= 0; i--) {
      ...
      var puckMesh = this.env.pucks.get(puckBody.index);
      ...
      //fire
      if( settings.data.debugFireball && !this.fireSkin.isActive ) {
        this.fireSkin.attachToMesh(puckMesh)
      }
      else if( !settings.data.debugFireball && this.fireSkin.isActive ) {
        this.fireSkin.detach()
      }
      else if( this.fireSkin.isActive) {
        this.fireSkin.update();
      }

       //ghost
      if( settings.data.debugGhostball && !this.ghostSkin.isActive ) {
        this.ghostSkin.attachToMesh(puckMesh)
      }
      else if( !settings.data.debugGhostball && this.ghostSkin.isActive ) {
        this.ghostSkin.detach()
      }
      else if( this.ghostSkin.isActive) {
        this.ghostSkin.update();
      }
