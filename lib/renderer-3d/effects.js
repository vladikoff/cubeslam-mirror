...
Effects.prototype = {
  ...
  update: function(world,alpha){
    ...
    //blink all ghost balls
    for (var i = world.pucks.values.length - 1; i >= 0; i--) {
      var puckBody = world.pucks.values[i];
      var puckMesh = this.env.pucks.get(puckBody.id);

      if( puckBody.flags & Body.GHOST ) {
        puckMesh.material.opacity = ( puckMesh.material.opacity == 1)?0:1;
      }
      else puckMesh.material.opacity = 1;
    };

    
    var showFog = world.activeExtras.get('fog')

    // check fog frame
    if( showFog ){
      // has it started?
      if( world.frame < showFog.start ){
        showFog = false

      // has it ended?
      } else if( world.frame > showFog.end ){
...
/*
  var allCubes = new THREE.Geometry();
  var cubeGeo = new THREE.CubeGeometry(settings.data.unitSize, settings.data.unitSize, settings.data.unitSize);
  var tempMesh = new THREE.Mesh(cubeGeo,null);
  for (var k = 2; k >= 0; k--) {
    for (var i = settings.data.arenaColumns-1; i >= 0; i--) {
      for (var j = 9; j >= 0; j--) {
        tempMesh.position.y = settings.data.unitSize*k + settings.data.unitSize*.5;
        tempMesh.position.x = (i*settings.data.unitSize+settings.data.unitSize*.5) - settings.data.arenaWidth*.5;
        tempMesh.position.z = -j*settings.data.unitSize + settings.data.unitSize*.5;
        THREE.GeometryUtils.merge(allCubes,tempMesh);
      };
    };
  }*/

  //var sliceMesh = new THREE.Mesh(allCubes, new THREE.MeshLambertMaterial({color:0xffffff, opacity:0.6, transparent:true, side:THREE.DoubleSide }) );
  //var sliceMesh = new THREE.Mesh(allCubes, Materials.fog );
