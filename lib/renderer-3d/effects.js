  ...
  update: function(world,alpha){
    ...
    for (var i = world.pucks.values.length - 1; i >= 0; i--) {
      ...
      if( BodyFlags.has(puckBody,BodyFlags.GHOST) ) {
        puckBody.data.blinkAmount = puckBody.data.blinkAmount ? 0 : 1;
        var hsv = puckMesh.material.color.getHSV();
        hsv.v += 0.4*(puckBody.data.blinkAmount)?1:-1;
        puckMesh.material.color.setHSV(hsv.h,hsv.s,hsv.v)


      } else {
        if( puckBody.data.blinkAmount  ) {
          //remove current color change 
          var hsv = puckMesh.material.color.getHSV()
          hsv.v -= 0.4;
          puckMesh.material.color.setHSV(hsv.h,hsv.s,hsv.v)
    ...
    for (var i = world.shields.values.length - 1; i >= 0; i--) {
      ...
      if( shieldBody.data.bulletproof && shieldMesh.material.opacity <= 1) {
    ...
    //death ball, temp for testing
    if( settings.data.debugDeathball && !this.deathballActive ) {

      this.deathballActive = true;
