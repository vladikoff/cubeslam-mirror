...
function Renderer(canvas){
  ...
  this.overlayMirror = createOverlayMirror( this.cameraController.camera);
...
*/
  ...
  onWindowresize: function(){
    ...
    this.overlayMirror.position.z = -h*0.1 /(2*Math.tan( this.cameraController.camera.fov*(Math.PI/360)) );
...
function createOverlayMirror( camera ) {

  var planeGeo = new THREE.PlaneGeometry(100,100,1,1);
  var plane = new THREE.Mesh(planeGeo, Materials.overlayMirror )

  TweenMax.to( plane.rotation,3.5,{delay:2,y:Math.PI,yoyo:true,repeat:-1,repeatDelay:2})
  TweenMax.to( plane.scale,3.5,{delay:2,x:0.5,y:0.5,z:0.5,yoyo:true,repeat:-1,repeatDelay:2})
