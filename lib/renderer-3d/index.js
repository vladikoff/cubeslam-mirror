...
Renderer.prototype = {
  ...
  activePlayer: function(id){
    ...
    if( id == 0 ){
      ...
      this.players.a.cube.rotation.y = 0;
  ...
  changeView : function( state ) { 
    ...
    if( state == "lobby") {
      ...
      this.initAngle = 0//Math.sqrt(x*x + z*z);// Math.PI*(( < 0 )?.5:1.5) + localTime*0.0005
  ...
  swapToVideoTexture : function() {
    ...
    new TWEEN.Tween(this.inactive.cube.position)
      .to( { y: -hh}, 500 )
      .onComplete(function(){
        this.materials.videoCube[4] = this.materials.remoteVideo;
        this.inactive.cube.geometry.materials = this.materials.videoCube;
        new TWEEN.Tween(this.inactive.cube.position)
          .to( { y: hh }, 1000 )
          .start();
      }.bind(this))
