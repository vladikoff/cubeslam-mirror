...
function Renderer(canvas){
  ...
  this.renderer = createRenderer(canvas,this.cameraController.camera)
  ...
  this.post = createPost(this.renderer,this.scene,this.cameraController.camera);
...
Renderer.prototype = {
  ...
  changeView : function( state , callback ) {
    ...
    if( state == "lobby" ) {
      ...
      this.time = Date.now();
      this.initAngle = Math.atan2(this.cameraController.camera.position.z, this.cameraController.camera.position.x) - Math.PI*.25;
      ...
      this.cameraUpdateFunc = function(){
        ...
        pos = this.cameraController.camera.target;
        pos.x += (this.env.center.x-pos.x)/24;
        pos.y += (this.env.center.y-pos.y)/24;
        pos.z += (this.env.center.z-pos.z)/24;

        this.cameraController.camera.lookAt(this.cameraController.camera.target);
  ...
  moveCamera : function(pointData){
    ...
    var originCurrent = this.cameraController.camera.position;
    ...
    var targetCurrent = this.cameraController.camera.target;
    targetCurrent.x += ((pointData.target.x+this.active.cameraOffset.x*2)-targetCurrent.x)/4;
    targetCurrent.y += (pointData.target.y-targetCurrent.y)/24;
    targetCurrent.z += (pointData.target.z-targetCurrent.z)/24;

    this.cameraController.camera.lookAt(targetCurrent);
  ...
  getWorldCoordinate: function(x,y) {
    ...
    var distance = (settings.data.arenaHeight*.5 - (this.cameraController.camera.position.z-this.active.cameraOffset.z)) / dir.z;
  ...
  render: function(world,alpha){
    if (this.players) {
      this.players.a.update(world.players.a,world.paddles.get(world.players.a.paddle))
      this.players.b.update(world.players.b,world.paddles.get(world.players.b.paddle))
    }

    if( settings.data.overrideCamera == "top") {
      this.cameraController.camera.position.set(300,3000,0);
      this.cameraController.camera.lookAt( new THREE.Vector3(300,0,0));
      this.cameraController.camera.rotation.z = Math.PI*.5
    } else {
