...
function Renderer(canvas){
  ...
  Materials.on("initDone", function(){
    this.emit("initDone");
  }.bind(this))
  ...
  this.extraIcons = new ExtraIcons();
  actions.on('showExtra', this.extraIcons.add.bind(this.extraIcons))
  actions.on('removeExtra', this.extraIcons.remove.bind(this.extraIcons))
  actions.on('activateExtra', this.extraIcons.activate.bind(this.extraIcons))
  ...
  settings.on("rendererChanged", function(){
...
function createRenderer(canvas){
  ...
  var w = window.innerWidth
    , h = window.innerHeight - $('#footer').height();
  $('#game').height(h);
  var renderer = new THREE.WebGLRenderer({canvas: canvas, antialias:settings.data.antialias});
...
var ExtraIcons = function(){
  this.container = $('#extras ul');
  this.clonables = this.container.children().detach();
}
ExtraIcons.prototype = {
  add: function(id, extra){
    if( this.getExtra(extra).length > 0 )
      return false;
    var el = this.clonables.filter('.'+id).clone();
  ...
  remove: function(id, extra) {
    var el = this.getExtra(extra).removeClass('visible active');
    setTimeout(function(){
      el.remove();
    }, 400)
    this.position();
  },
  activate: function(id, extra){
    this.getExtra(extra).addClass('active');
  },
  reset: function(){
    this.container.children().remove();
  },
  position: function(){
    this.container.children('.visible').each( function(i){
      var y = i * 52;
      $(this).css('top', y).addClass('visible');
    })
  },
