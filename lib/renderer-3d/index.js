...
Renderer.prototype = {
  ...
  onWindowresize: function(){
    ...
    this.overlay.scale.set( w/100, h/100,1);
    this.overlay.position.z = -h /(2*Math.tan( this.cameraController.camera.fov*(Math.PI/360)) );
    //this.overlay.position.z = h / (2 * Math.tan(45 / 2 * (Math.PI / 180)));
...
*/
  ...
  // var tapeTexture = THREE.ImageUtils.loadTexture( "images/ash_uvgrid01.jpg" );
  tapeTexture.mapping = THREE.UVMapping;

  var plane = new THREE.Mesh(planeGeo, new THREE.MeshBasicMaterial({blending:THREE.MultiplyBlending, map:tapeTexture,transparent:true, wireframe:false}))
