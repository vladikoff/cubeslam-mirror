  ...
  changeView : function( state ) {
    ...
    if( state == "lobby") {
      ...
      this.cameraUpdateFunc = function(){
        ...
        /*if( swapTimestamp + 4000 < Date.now() ){
          swapTimestamp = Date.now();
          changeSymbolTween.start();
        }*/
      ...
      if( this.active )
        this.active.cameraOffset.x = 20*(this.active.paddle.position.x/world.bounds.width);
...
function createPost(renderer,scene,camera) {
  ...
  var effectAA = new THREE.ShaderPass( THREE.FXAAShader );
