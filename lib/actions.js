...
function create(type /*, ...*/){
  ...
  switch(type){
    ...
    case Actions.PADDLE_MOVE:  // paddleIndex, x (number between 0 and 1)
      ...
      send = this.physics.frame % 3 == 0 ? 2 : 0; 
      ...
      puck.onbounds = function(at){
        ...
        if( at.y <= 0 ){
          ...
          if( at.x >= x - hw && at.x <= x + hw ){
            ...
            this.world.guestHits.push(at.x);
  ...
  if( send ){
    var msg = [type];
    (send === 1) && msg.push(this.physics.frame)
    msg.push(opts.join('‚')) // note: not a regular comma (alt-comma)
    this.channel.send(msg.join('·')) // note: alt-shift-period
  }
}
