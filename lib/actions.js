...
exports.puckCreate = function(x, y, mass){
  ...
  puck.collidables = world.obstacles; 
  puck.oncollision = function(at,poly){
    console.log('puck [%s] collision %s,%s',puck.id,at.x,at.y);

    // was the colliding polygon a paddle?
    if( ~world.paddles.indexOf(poly) ){
      console.log('puck hit a paddle!!!')
  ...
  puck.onbounds = function(at){
    ...
    if( player ){
      ...
      debug('hit [%s] %s',puck.id,player.name)
...
exports.obstacleCreate = function(vertices){
  // convert the vertices to a polygon
  var poly = new Polygon(vertices).reverse();
  world.obstacles.push(poly);

  // send it as reliable if host
  if( world.host ){
    buffer.push(['oc', world.frame, [vertices]])
  }
}
