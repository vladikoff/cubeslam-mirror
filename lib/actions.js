...
exports.paddleMove = function(i, x){
  ...
  if( (world.host && i == 0) || (!world.host && i == 1) )
    buffer.push(['m', world.frame, slice(arguments)])
}
...
exports.puckCreate = function(x, y, mass){
  ...
  puck.oncollision = function oncollision(at,poly){
    ...
    if( ~world.paddles.indexOf(poly) ){
      ...
      // TODO push the puck in the velocity direction multiplied
      //      by player.power (ie. Point.normalize(velocity).mul(player.power))
...
function parse(msg,fn){
  msg.split('\n').forEach(function(msg){
    var parts = msg.split('·');
    fn(parts[0], parts[1] ? +parts[1] : null, parts[2] ? parts[2].split('‚') : null)
  })
}
