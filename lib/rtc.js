...
exports.connect = function(token,room,clientId){
  ...
  socket.onmessage = function(msg){
    ...
    switch(json.Type){
      ...
      case 'offer':
        ...
        connection.setRemoteDescription(desc,setOffer,function(err){
      ...
      case 'answer':
        ...
        connection.setRemoteDescription(desc,setAnswer,function(err){
          ...
          connection = rtc.connection = new PeerConnection(exports.configuration);
          connection.setRemoteDescription(desc,setAnswer,function(err){
            console.error('could not set remote description from answer',desc.sdp,err,connection)
          });
        });
        break;
        ...
        if (cands.length) {
          for (i in cands) {
            try {
              ...
              // TODO is there a better way than a try/catch?
              console.warn('got icecandidates too early',json.Data,e)
            }
          }
        } else {
          try {
            var candidate = new RTCIceCandidate(cands);
            connection.addIceCandidate(candidate);
          } catch(e) {
            // TODO is there a better way than a try/catch?
            console.warn('got icecandidates too early',json.Data,e)
      ...
      case 'participants':
        if (roomParticipants != json.Data) {
          ...
          // Set the peer (the first in the list that is not me)
          socket.peer = null;
          for (i in newParts) {
  ...
  */
  ...
  connection.onicecandidate = function(e){
    ...
    */
    if (e.candidate && socket.peer) {
      debug.connection('icecandidate',socket.peer)
