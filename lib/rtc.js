...
exports.connect = function(token,room,clientId){
  ...
  socket.onmessage = function(msg){
    ...
    switch(type){
      ...
      case 'disconnected':
        ...
        if( data === socket.peer ) // ignore own disconnect
          rtc.emit('disconnect',data);
        delete socket.peer;
        break;
  ...
  socket.send = function(type,data){
    ...
    req.open('POST', '/message?'+Math.random(), true);
    req.setRequestHeader('Content-Type','application/x-www-form-urlencoded')
    req.send(
      'from='+encodeURIComponent(room+'@'+clientId) +
  ...
  window.onunload = function(e){
    ...
    req.setRequestHeader('Content-Type','application/x-www-form-urlencoded')
    req.send('from='+room+'@'+clientId);
  }

  var sendOffer = function(){
    if( socket.peer && ~['active','new'].indexOf(connection.readyState) ){
      ...
      debug.connection('not sending offer (readystate:%s)',connection.readyState)
    }
  }
