...
settings.data = {
    ...
    renderer: "3d",
    maxHits: 10,
    keyboardSpeedMax: 100,
    keyboardAccelerate: 0.98,
...
function createGenericUI() {
 ...
 //stats
  ...
  f.add(settings.data, 'fpsCamera').name('FPS Camera');
  ...
  f = gui.addFolder('Keyboard settings');
  f.add(settings.data, 'keyboardSpeedMax').min(1).max(100).name("Speed");
  f.add(settings.data, 'keyboardAccelerate').min(0).max(1).name("Acceleration");
  f.add(settings.data, 'keyboardDamping').min(0).max(1).name("Damping");
  //f.add(world, 'keyboardSpeed').name("Current speed").listen();
