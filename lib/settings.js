...
settings.createGenericUI = function( initParams ) {
  ...
  f.add(settings.data, 'cameraType',{'Scripted':0,'FPS':1,'Classic':2,'Rabbit cam':3}).name('Mode').listen().onChange(function(value){
    settings.data.cameraType = parseInt(value);
    this.emit("cameraTypeChanged")
  }.bind(this));
  /*f.add(settings.data,"antialias").onChange(function(){
...
*/
  ...
  f.add(settings,"toggleInfo").name("Debug panel")
  ...
  if( initParams.isMobile ) {
    ...
    f.add(settings.data, 'extraSaturate').min(0).max(100).name("Extras Hue").onChange(mobileExtrasColor);
  }
