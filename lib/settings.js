var audio = require('./audio')
  , Emitter = require('emitter')
  , world = require('./world');

var settings = exports;

Emitter(settings);

settings.level = {

    ballMaxSpeed: 10,
    
    theme: {
        shieldColor: 0xffffff,
        arenaColor: 0xaa1000,
        puckColor: 0xefce06,
        terrainColor1: 0x4d87dc,
        terrainColor2: 0x1f84d5,
        terrainColor3: 0x195475,
        treeBranchColor: 0x1564a4,
        treeTrunkColor: 0x1564a4,
        iconColor: 0x3c1ccf
    }

}


settings.levelData =
{
  "remembered": {
    "Default": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 11145216,
        "terrainColor1": 5081052,
        "terrainColor2": 2065621,
        "terrainColor3": 1660021,
        "treeBranchColor": 1402020,
        "iconColor": 0x3c1ccf
      }
    },
    "level1": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 12463148,
        "terrainColor1": 3374054,
        "terrainColor2": 1407434,
        "terrainColor3": 2584018,
        "treeBranchColor": 3633369,
        "iconColor": 0x3c1ccf
      }
    },
    "level2": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 3437519,
        "terrainColor1": 5674547,
        "terrainColor2": 4558365,
        "terrainColor3": 4822547,
        "treeBranchColor": 4296982,
        "iconColor": 0x3c1ccf
      }
    },
    "level VIT": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 6714485,
        "terrainColor1": 14934989,
        "terrainColor2": 15131078,
        "terrainColor3": 14145467,
        "treeBranchColor": 14802639,
        "iconColor": 0x3c1ccf
      }
    },
    "LEVEL GREEN": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 4693302,
        "terrainColor1": 3369162,
        "terrainColor2": 1399980,
        "terrainColor3": 2249624,
        "treeBranchColor": 1338555,
        "iconColor": 0x3c1ccf
      }
    },
    "LEVEL BLACK": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 1250068,
        "terrainColor1": 3618615,
        "terrainColor2": 2829357,
        "terrainColor3": 2368808,
        "treeBranchColor": 3422268,
        "iconColor": 0x3c1ccf
      }
    },
    "Level Orange": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 15259350,
        "puckColor": 15790303,
        "arenaColor": 12477202,
        "terrainColor1": 1218007,
        "terrainColor2": 2327523,
        "terrainColor3": 5723991,
        "treeBranchColor": 3642849,
        "iconColor": 0x3c1ccf
      }
    },
    "Level Yellow": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 6247213,
        "terrainColor1": 5732410,
        "terrainColor2": 3957776,
        "terrainColor3": 5468189,
        "treeBranchColor": 5333290,
        "iconColor": 0x3c1ccf
      }
    },
    "Level 8": {
      "0": {
        "ballMaxSpeed": 10
      },
      "1": {
        "shieldColor": 16777215,
        "puckColor": 15715846,
        "arenaColor": 2896930,
        "terrainColor1": 5732410,
        "terrainColor2": 5271598,
        "terrainColor3": 5466917,
        "treeBranchColor": 5333290,
        "iconColor": 0x3c1ccf
      }
    }
  },
  "preset": "level1",
  "closed": false,
  "folders": {
    "theme": {
      "preset": "Default",
      "closed": false,
      "folders": {}
    }
  }
}


settings.data = {

    showDebugInfo: true,

    // game
    inputType: "keyboard",
    arenaWidth: 1700,
    arenaHeight: 2200,
    arenaSideHeight: 150,
    paddleDepth:100,
    videoBoxDepth:700,
    paused: false,
    reversed: false,
    renderer: "3d",
    maxHits: 3,
    keyboardSpeedMax: 31,
    keyboardAccelerate: 0.95,
    keyboardDamping: 0.76,
    music: false,
    sounds: true,

    // info
    frame: 0,
    speed: 0.000001,

    // motion tracking
    useMotionTracking:true,
    trackingColorPreBlur: 20,
    trackingColor: [102,24,37],
    trackingColorPreview: [102,24,37],
    trackingColorThreshold: 0.32,
    trackingColorAutoThreshold: true,
    trackingColorBlur: 20-1.6,
    trackingColorBlurThreshold: 0.1,
    motionBlur:20-3,
    motionThreshold:0.03,
    pointSmoothing:10,

    dirLightColor: 0xffffff,
    dirLightIntensity: 0.80 ,
    hemisphereLightSkyColor: 0xffffff,
    hemisphereLightGroundColor: 0x000000,
    hemisphereLightIntensity:0.8,
    pointLightColor: 0xffffff,
    pointLightIntensity: 0.6,

    useShadows:false,
    usePost:true,
    puckRadius: 20,
    arenaSurfaceY: -200,
    arenaColumns: 18,

    // networking
    countdown: 1000
}

createGenericUI()
//createLevelUI()
lightsUpdated()

function createGenericUI() {
  // dat.GUI is global, included in the HTML
  var gui = settings.gui = new dat.GUI({ autoPlace: false });
  gui.width = 332;
  document.getElementById("settingsDataGUI").appendChild(gui.domElement);

  //stats
  settings.stats = new Stats();
  //document.getElementById("statsDebug").appendChild(settings.stats.domElement);

  var f;

  f = gui.addFolder('Game settings');
  f.add(settings.data, 'showDebugInfo').name("show debug panel").onChange(function(value){
    $("#debug-info").toggle(value);
  });

  f.add(settings.data, 'paused').name('Game Paused');
  f.add(settings.data, 'arenaWidth').min(400).name('Arena width');
  f.add(settings.data, 'arenaHeight').min(400).name('Arena length');
  f.add(settings.data, 'inputType').options("mouse","keyboard").name("Input Type").listen();
  f.add(settings.data, 'keyboardSpeedMax').min(1).max(40).name("Keyboard speed");
  f.add(settings.data, 'keyboardAccelerate').min(0).max(1).name("Keyboard acc");
  f.add(settings.data, 'keyboardDamping').min(0).max(1).name("Keyboard damping");
  f.add(settings.data, 'usePost').name("use post processing");


  f.add(settings.data, 'music').name("Play soundtrack").onChange(function(value){
    if( value ) {
      audio.play("soundtrack");
    } else {
      audio.stop("soundtrack");
    }
  });

  //stats
  settings.stats = new Stats();
  //document.getElementById("statsDebug").appendChild(settings.stats.domElement);


  f = gui.addFolder('Lights and colors');
  f.addColor(settings.data, 'dirLightColor').name("dir color").onChange(lightsUpdated);
  f.add(settings.data, 'dirLightIntensity').min(0).max(1).name("Dir intensity").onChange(lightsUpdated);
  f.addColor(settings.data, 'hemisphereLightSkyColor').name("Hemisphere sky color").onChange(lightsUpdated);
  f.addColor(settings.data, 'hemisphereLightGroundColor').name("Hemisphere ground color").onChange(lightsUpdated);
  f.add(settings.data, 'hemisphereLightIntensity').min(0).max(1).name("Hemisphere intensity").onChange(lightsUpdated);


  f = gui.addFolder('Stats');
  f.add(world, 'frame').name('Frame').listen();
  f.add(world, 'collisions').name('Collisions').listen();

  gui.close();

  //ugly hack to change title of menu
  $("#settingsDataGUI").find(".close-button").click( function(){
    $(this).html("Global")
  }).html("Global");

//$("#settingsGUIContainer").hide()
}
/*
function createLevelUI() {
  // set level1 as default until the game starts
  // (by which we'll have the level editor and
  // the puppeteer to take care of it...)
  settings.level = require('./levels/level1.js');

  var f;

  var extras = Object.keys(require('./extras'));
  var actions = Object.keys(require('./geom-sim/actions')).filter(function(a){
    return a.length > 4
        && ( a.indexOf('Create') != -1
          || a.indexOf('Destroy') != -1
          || a == 'gameOver' )
  });
  var events = {
    count: 1,
    play: function(){
      // TODO somehow load this into the puppeteer and then switch
      //      to GameStartState
      console.log('play this level!')
    },
    add: function(){
      var params = [];
      var name = 'Event '+this.count++;
      var f = gui.addFolder(name)
      var event = {
        action: '',
        time: 0,
        params: [],
        remove: function(){
          // remove any params
          while(params.length)
            f.remove(params.pop());
          // remove from dom
          gui.__ul.removeChild(f.domElement.parentElement)
          // remove from folders
          delete gui.__folders[f.name]
        }
      };
      f.add(event,'remove').name('Delete event')
      f.add(event,'action').options([''].concat(actions)).onChange(function(action){
        // remove any previous params
        while(params.length)
          f.remove(params.pop());

        // rename the event to include the action name
        var title = name;
        if( action )
          title += ' - ' + action.replace(/([A-Z])/g,' $1');
        $('li.title',f.__ul).text(title)

        // all events has a time property
        // TODO default to last events time?
        params.push(f.add(event,'time').min(0).name('Time'))

        // update the params
        switch(action){
          case '': return; // nothin...

          // create params for kind, x, y and mass
          case 'forceCreate':
            event.params = ['',0,0,1];
            params.push(f.add(event.params,'0').options('repell','attract').name('Kind'))
            params.push(f.add(event.params,'1').min(0).max(settings.data.arenaWidth).name('X'))
            params.push(f.add(event.params,'2').min(0).max(settings.data.arenaWidth).name('Y'))
            params.push(f.add(event.params,'3').name('Mass'))
            break;

          // create params for kind, x, y
          case 'forceDestroy':
            event.params = ['',0,0];
            // TODO make a select box instead which only contains existing forces
            params.push(f.add(event.params,'0').options('repell','attract').name('Kind'))
            params.push(f.add(event.params,'1').min(0).max(settings.data.arenaWidth).name('X'))
            params.push(f.add(event.params,'2').min(0).max(settings.data.arenaWidth).name('Y'))
            break;

          // create params for id, x and y
          case 'obstacleCreate':
          case 'extraCreate':
            event.params = ['',0,0];
            params.push(f.add(event.params,'0').options(extras).name('Id'))
            params.push(f.add(event.params,'1').min(0).max(settings.data.arenaWidth).name('X'))
            params.push(f.add(event.params,'2').min(0).max(settings.data.arenaWidth).name('Y'))
            break;

          // create params for id
          case 'extraDestroy':
          case 'obstacleDestroy':
            event.params = [''];
            // TODO only ids that's already in use
            var ids = extras.filter(function(){

            })
            // TODO if no ids are available. alert and return?
            params.push(f.add(event.params,'0').options(ids).name('Id'))
            break;

          // create params for x and y
          case 'puckCreate':
            event.params = [0,0];
            params.push(f.add(event.params,'0').min(0).max(settings.data.arenaWidth).name('X'))
            params.push(f.add(event.params,'1').min(0).max(settings.data.arenaWidth).name('Y'))
            break;
        }
      })
    }
  }

  gui.add(events,"play").name("Play Level")
  gui.add(settings.level, 'ballMaxSpeed').min(1).max(20);

  f = gui.addFolder('theme');
  f.addColor(settings.level.theme, 'shieldColor').name("Shield hit").onChange(colorsUpdated);
  f.addColor(settings.level.theme, 'puckColor').name("Puck").onChange(colorsUpdated);
  f.addColor(settings.level.theme, 'arenaColor').name("Arena").onChange(colorsUpdated);
  f.addColor(settings.level.theme, 'terrainColor1').name("Terrain1").onChange(colorsUpdated);
  f.addColor(settings.level.theme, 'terrainColor2').name("Terrain2").onChange(colorsUpdated);
  f.addColor(settings.level.theme, 'terrainColor3').name("Terrain3").onChange(colorsUpdated);
  f.addColor(settings.level.theme, 'treeBranchColor').name("Trees").onChange(colorsUpdated);
  f.addColor(settings.level.theme, 'iconColor').name("Icons").onChange(colorsUpdated);
  //f.addColor(settings.level.theme, 'treeTrunkColor').name("Tree trunks").onChange(colorsUpdated);

  gui.add(events,"add").name("Add Event")
  gui.close();

  $("#settingsLevelsGUI").find(".close-button:last").click( function(){
    $(this).html("Levels")
  }).html("Levels")
//$("#settingsGUIContainer").hide()

}
*/
function colorsUpdated( value) {
  settings.emit('colorsUpdated');
}

function lightsUpdated( value ) {
  settings.emit('lightsUpdated');
}

