...
exports.start = function(context){
  ...
  ctx.network.remote.on('hashes',function(e){
    var frames = Object.keys(e.hashes).concat(Object.keys(hashes)).sort(function(a,b){return parseInt(a)-parseInt(b)});
    console.group('comparing hashes')
    ...
    // clear the hashes and jsons
    hashes = {}
    jsons = {}
    console.groupEnd('comparing hashes')
  })
  ctx.network.remote.on('world',function(e){
    console.group('comparing worlds')
    if( jsons[e.frame] !== e.world ){
      console.log('NOT THE SAME, trying diff:')
      console.log(diff.createPatch('diff',jsons[e.frame],e.world,'local','remove'))
    }
    jsons = {}
    console.groupEnd('comparing worlds')
  })
  ...
  ctx.networkGame.on('post update',function(world){
    // hash and store without me/opponent
    var me = world.me
      , op = world.opponent;
    delete world.me;
    delete world.opponent;
    hashes[world.frame] = hashCode(world)
    jsons[world.frame] = JSON.stringify(world,null,2)
    world.me = me;
    world.opponent = op;
  })
...
function flush(){
  ...
  if( !sendingAck && Math.abs(ctx.game.world.frame - lastFrame) > 10 ){
    sendingAck = true;
    ctx.game.emit('input',World.ACK)
  }
}