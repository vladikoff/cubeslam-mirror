...
exports.start = function(context){
  ...
  actions.on('round over',roundOver)
}
...
exports.stop = function(){
  ...
  actions.off('round over',roundOver)
  keys.unbind('c')
  keys.unbind(',')
  keys.unbind('.')
  keys.unbind('f')
...
function onmessage(buf){
  ...
  forward('network',ctx.sync,Math.min(ctx.game.world.frame,frame)) && replay()
}

var seq = 0;
function oninputs(world,inputs,size){
  if( !size )
    return;// debug('skipping apply. no input: %s',inputs);
  if( isReplaying )
  ...
  forward('inputs',ctx.sync,Math.min(ctx.sync.world.frame,world.frame)) && replay()
}

function manualForward(){
  debug('manual forward to %s',ctx.game.world.frame);
  return forward('manual',ctx.sync,ctx.game.world.frame)
}


function forward(src,game,frame){
  // update the sync with both
  ...
  return !waitForGameOver;
}


function replay(){
  var fromGame = ctx.sync
    , toGame = ctx.game
  ...
  var rLocalQueue = localQueue.slice()
    , rNetworkQueue = networkQueue.slice();
...
function checkQueue(name,q,game){
  ...
  if( q[0] && q[1] !== World.ACK && game.world.frame > q[0] ){
...
function flush(){
  ...
  if( ctx.game.world.state === World.PLAYING
   && Math.abs(ctx.game.world.frame - lastFrame) > 10 ){
...
function roundOver(world){
  ...
  if( world.name === 'sync' ){
    ...
    // mark it as game over
    // actions.gameOver(world);
    // actions.gameOver(ctx.game.world);

    // needed so we don't replay() which
    // will delete all the inputs!
