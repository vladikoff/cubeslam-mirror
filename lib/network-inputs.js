...
exports.start = function(context){
  ...
  ctx.network.remote.on('round over',function(){
    debug('network round over',roundOver)
  })
  actions.on('round over',function(world){
    debug('local round over',roundOver,world.name)

    // both games agree on round over, go on!
    if( world.name === 'networkGame' ){
      // emit ACK to make sure the other player
      // will forward up to this point
      ctx.game.emit('input',World.ACK)

      // when agreed, both worlds are over
      ctx.game.world.over = world.over = true
...
function oninputs(world,inputs,size){
  if( !size ) return;// debug('skipping apply. no input');
  if( isReplaying ) return debug('skipping apply. is replaying');
  if( world.over ) return debug('skipping apply. is over');
...
function manualForward(){
  ...
  return forward(ctx.networkGame,ctx.game.world.frame)
}
...
function forward(game,frame){
  ...
  if( game.world.over ){
    debug('skipping forwarding (to %s). is over',frame);
    return false;
  }
