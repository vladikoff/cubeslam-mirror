...
function MotionTracker(){
  ...
  tracker.defaultBB = new BoundingBox(INPUT_WIDTH,INPUT_HEIGHT)
...
//    smoothedMarkersListY.unshift(markerToY);
  ...
  tracker.blobDetect = function() {
    ...
    if( motionPixelsFound < colorPixelsFound && tracker.colorBB.width < 40 ) {
      tracker.readBB = tracker.colorBB;
    }
    //else use combo
    else if( mainPixelsFound > 50 ) {
