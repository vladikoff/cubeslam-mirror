...
function Simulator(canvas,form){
  ...
  this.world = {
    //accumulated hits, reset on game over/set
    hostHits: [],
    guestHits: []
  }

  //if( !renderer ) renderer = new Renderer(canvas,this.bounds,this.world);
...
Simulator.prototype = {
  ...
  create: function(){
    ...
    this.createPaddle(1); // 0 means "top" (i.e. height*0)
    this.createPaddle(0); // 1 means "bottom" (i.e. height*1)
