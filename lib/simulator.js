...
Simulator.prototype = {
  ...
  createPuck: function(){
    ...
    actions.puckPush(lastPuck.id, 0, -90)
  },
  ...
  controls: function(inputs){
    ...
    if( settings.data.inputType == "motion" && inputs.motion.moved ){
      actions.paddleMove(world.me.paddle.id, world.host ? inputs.motion.x : 1-inputs.motion.x);

    } else if( settings.data.inputType == "mouse" && inputs.mouse.moved ){
      actions.paddleMove(world.me.paddle.id, world.host ? inputs.mouse.x : 1-inputs.mouse.x);
