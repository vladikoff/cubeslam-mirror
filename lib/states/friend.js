  ...
  enter: function(ctx){
    ...
    ctx.network.on('change pathname',function(pathname){
      if( pathname == '/friend/waiting' )
        see('/friend/accept');
      else if( pathname == '/friend/arrived' )
        see('/friend/accept')
      else if( pathname == '/friend/invite' )
        see('/friend/arrived')
      else
        console.warn('friend invite unexpected pathname:',pathname)
    })
...
exports.Left = {
  enter: function(ctx){
    $('.play-friend',ctx.el).on('click',function(){ see('/main-menu') })
    $('.play',ctx.el).on('click',function(){ see('/game/instructions') })
    inputs.bind('space',function(){ see('/game/instructions') })
  },
  leave: function(ctx){
    $('.play-friend',ctx.el).off('click')
