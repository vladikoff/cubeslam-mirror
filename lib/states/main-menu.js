...
MainMenu.enter = function(ctx){
  ...
  //prevent GA tracking first time
  sound(cookie('sound'),false);
  $('.sound-switch').click(function(){sound(true)})
  ...
  if( ctx.query.god ) settings.data.godMode = true;

  if( ctx.query.dev ) {
     _gaq.push(['_trackEvent', 'settings', 'dev enabled']);
    settings.createGenericUI()
    settings.on("addPuck", function(){
      actions.createPuckCenter(ctx.game.world)
    })
  }
...
function sound(toggle,trackGA){
  ...
  switch(toggle){
    case 'on':
      ...
      if( trackGA) _gaq.push(['_trackEvent', 'sound', 'on']);
    ...
    case 'off':
     if( trackGA ) _gaq.push(['_trackEvent', 'sound', 'off']);
