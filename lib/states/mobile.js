...
Mobile.enter = function(ctx){
  ...
  ctx.mobile = true;
  //Adding back since it keep crashing chrome on android 
  ctx.silent = true;
  ...
  $(window).on('resize', function(){
    var rect = {w: 560, h: 500}
      ...
      , h = dh / rect.h
      , scale = (w > h) ? h : w;
    c.webkitTransform = c.msTransform = c.MozTransform = c.OTransform = 'scale('+scale+') translateZ(0)';
    ...
    if( scale < 1.4)
      settings.data.mouseSensitivity = 1.8-scale;
  }).resize();
}
