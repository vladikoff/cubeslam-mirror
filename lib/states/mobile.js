...
Mobile.enter = function(ctx){
  ...
  $('#canv-css .background img.bg')
    .attr('src', $('#canv-css .background img.bg').data( $(document).width() > 800 ? 'src-tablet' : 'src-mobile'));
  ...
  $(window).on('resize', function(){
    ...
    if( scale < 1)
      settings.data.mouseSensitivity = 1.4-scale;
