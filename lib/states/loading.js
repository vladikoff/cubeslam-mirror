...
Loading.leave = function(ctx, next){
  ...
  if(!ctx.silent){
    this.loader.push(function(done){
      ...
      dmaf.addEventListener('dmaf_fail', done);
      dmaf.init();
    })
  }
  ...
  if(ctx.mobile){
    // load CSS renderer
    this.loader.push(function(done){
      loadScript('/javascript/renderer-css.js',function(err){
        console.log('loaded script!',arguments)
    ...
    this.loader.push(function(done){
      var pending = 2;
      loadScript('/javascript/libs/three.min.js',function(err){
        if( err ) return done(err);
        --pending || init()
      })
      loadScript('/javascript/renderer-3d.js',function(err){
        if( err ) return done(err);
        --pending || init()
      })
      function init(){
