...
Setup.enter = function enterSetupSync(ctx){
  ctx.renderer = new Renderer(document.getElementById('canv-2d'))

  ctx.game = new Game(ctx.renderer);
  ctx.game.world.name = 'game'

  // a test clone of Game, should
  // be equal if it receives the same input
  ctx.test = new Game();
  ctx.test.world.name = 'test'
  // ctx.game.on('input',ctx.test.emit.bind(ctx.test,'input'))
  // ctx.game.on('update',function(world,dt,t){
  //   ctx.test.update()
  // })
