...
Setup.enter = function(ctx){
  ...
  ctx.dev = location.hostname.indexOf('appspot.com') == -1;
...
function setupNetwork(ctx){
  ...
  if( !ctx.mobile && ctx.network.available ){
    ...
    // update debug information every message
    ctx.network.on('message',function(){
      $('#info-game-world').text(ctx.game.world.toString())
      $('#info-sync-world').text(ctx.sync && ctx.sync.world.toString())
      $('#info-netchan').text(ctx.network.game && ctx.network.game.toString())
      $('#info-datchan').text(ctx.network.game && dcInfo(ctx.network.game.channel))
      $('#info-peercon').text(ctx.network.remote && pcInfo(ctx.network.remote.connection))
      $('#info-inputs').text(inputs.toString())
    })
    ...
    ctx.network.on('error', function(e){
      ...
      switch(e.code){
        case 1168: // message too long (1168 = max length)
          console.log(ctx.game.world.toString())
          console.log(ctx.sync.world.toString())
          console.log(ctx.network.game.toString())
          console.log(inputs.toString())
          $('#info-game-world').text(ctx.game.world.toString())
          $('#info-sync-world').text(ctx.sync.world.toString())
          $('#info-netchan').text(ctx.network.game.toString())
          $('#info-inputs').text(inputs.toString())
...
function setupBindings(ctx){
 ...
 /* keys.bind('up up down down left right left right b a enter', function() {
  ...
  keys.bind('b',function(){ actions.createPuckCenter(ctx.game.world) })
