...
Setup.enter = function(ctx){
  ...
  if( ctx.query.quality ){
    if(ctx.query.quality === settings.QUALITY_HIGH || ctx.query.quality === settings.QUALITY_LOW || window.devicePixelRatio > 1) {
      settings.data.quality = ctx.query.quality;  
    }

    if(settings.data.quality == settings.QUALITY_LOW)
...
function setupNetwork(ctx){
  ctx.network = new Network(ctx, !ctx.mobile)
  console.log("available", ctx.network.available)
  ...
  if( !ctx.mobile && ctx.network.available ){
    ...
    ctx.network.on('change latency',function(latency){
      ...
      if(quality == 'bad') {
        var notification = $('#latency:not(.inactive)').addClass('active');
        if(notification.length > 0)
          setTimeout(function(){
            notification.removeClass('active').addClass('inactive');
          },4000)
      }
...
function setupBindings(ctx){
  ...
  // input bindings
  keys.bind('o',function(){ $("#settingsGUIContainer,#debug-info").toggle(); info(ctx,$("#debug-info").is(':visible')) })
