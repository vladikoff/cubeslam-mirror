...
exports.Level = {
  enter: function(ctx){
    ...
    $('.state.game-prompt').find('.background').css("background","#" + new THREE.Color(Themes.current.countdown1).getHexString());
  },
...
exports.Start = {
  enter: function(ctx){
    ...
    $(".state.game-prompt").find(".background").css("background","#" + new THREE.Color(Themes.current.countdown2).getHexString());
    see(path||'/game/start')
  },
  leave: function(ctx,next){
    this.timeout = setTimeout(next,BEAT)
