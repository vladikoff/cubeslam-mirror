...
exports.Setup = {
  enter: function(ctx){
    ...
    ctx.renderer.activePlayer(ctx.network.winner ? 0 : 1, false, ctx.multiplayer)
    ...
    game.on('enter frame',function(world){
      var sent = inputs.network.flush()
        , n = now();
      if( sent ){
        lastSent = n;
      } else if( world.state == World.PLAYING && n - lastSent > 250 ){
    ...
    game.on('leave frame',function(world){
      ...
      if( ctx.sync.world.state != state || ctx.sync.world.frame != frame ){
