...
exports.Setup = {
  enter: function(ctx){
    ...
    if( ctx.multiplayer ){
      ...
      this.cleanupMultiplayer = setupMultiplayer(ctx)

      // set the active player in the renderer
      ctx.renderer.activePlayer(ctx.network.winner ? 0 : 1, false, true)

    } else {

      // set the active player in the renderer
      ctx.renderer.activePlayer(0, false, false)
    }
    ...
    this.cleanupInputs = setupInputs(ctx)
  ...
  leave: function(ctx){
    ...
    inputs.reset()
    this.cleanupMultiplayer && this.cleanupMultiplayer()
    this.cleanupInputs()
...
function setupInputs(ctx){
  var game = ctx.game
    , world = game.world
    , isLeft = 0
    , isRight = 0;

  game.on('pre update',onupdate)
  ...
  return function cleanup(){
    game.off('pre update',onupdate)
    game.off('pre update',inputs.process)
