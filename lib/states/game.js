...
exports.Setup = {
  enter: function(ctx){
    ...
    game.on('pre update',inputs.update)
  ...
  leave: function(ctx){
    ...
    inputs.reset()
    inputs.pause(true)
    ctx.game.off('pre update',inputs.update)
...
exports.Pause = {
  enter: function(ctx){
    ...
    this.timeout = setTimeout(function(){
      ...
      keys.bind('space', see.bind('/game/prompt/start'))
      keys.bind('esc', see.bind('/main-menu'))
      $('button.play').on('click',see.bind('/game/prompt/start'))
    }, 1000);
  },
  leave: function(ctx){
    clearTimeout(this.timeout);
