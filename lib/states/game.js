...
exports.Setup = {
  enter: function(ctx){
    ...
    if( ctx.multiplayer ){
      ...
      game.on('enter frame',function(world){
        ...
        if( n - lastSent > sendRate ){
          if( inputs.network.flush() ){
            ...
            sent[id] = now();
            inputs.record(inputs.types.EMIT,'ping',id);
          }
        }
      })
      ...
      var logTime = false
        , logProf = false;
      keys.bind('t',function(){logTime = true})
      keys.bind('y',function(){logProf = true})

      game.on('leave frame',function(world){
        var state = ctx.sync.world.state
          , frame = ctx.sync.world.frame;

        logTime && console.time('bah')
        logProf && console.profile('bah')
        ...
        // or replay!
          ...
          // ctx.sync.world.pucks.del('hack')
          //
          inputs.network.replay(ctx.sync.world,world,Math.abs(diff))
          replay = false;

          // interpolate the opponent paddle
          // if( paddleX !== null ){
          //   var p = world.paddles.get(world.opponent.paddle);
          //   p.target = {
          //     position: vec.make(paddleX,p.current[1]),
          //     frames: paddleFrame - world.frame
          //   }
          //   console.log('interpolate',p.target.frames,p.target.position)
          // }


          // ctx.sync.world.pucks.set('hack',ctx.game.world.pucks.values[0])
        }

        // end of silence
        dmaf.tell = tell;

        logTime && console.timeEnd('bah')
        logProf && console.profileEnd('bah')
        logTime = logProf = false
      })
      ...
      inputs.network.on('message',function(buf){
        ctx.network.send(buf)
      })
      inputs.network.on('ping',function(id){
        inputs.record(inputs.types.EMIT,'pong',id)
      })
      inputs.network.on('pong',function(id){
        var p = sent[id];
      ...
      inputs.network.on('move',function(id,x){
        if( id === ctx.game.world.opponent.paddle ){
          actions.movePaddle(ctx.game.world,id,x)
        }
      })
      inputs.network.on('hit',function(x,f){
        console.log('received hit %s. opponent paddle will be hit at x: %s',f,x,ctx.game.world.frame)
        // var cx = ctx.game.world.paddles.get(ctx.game.world.opponent.paddle).current[0];
        // var dx = x - cx;
        // console.log('opponent paddle current position:',cx)
        // console.log('opponent paddle diff:',dx)
        // console.log('move over %s frames?',Math.ceil(ctx.latency/16.6))
        var paddle = ctx.game.world.paddles.get(ctx.game.world.opponent.paddle);
        var puck = ctx.game.world.pucks.values[0];
        var dist = puck.current[1] - paddle.current[1];
        console.log('distance between puck and paddle. %spx %sframes',dist,(dist/puck.velocity[1])/(1000/60))
        // replay = true;
        paddle.previous[0] = paddle.current[0] = x;
        paddleX = x;
        paddleFrame = f;

        actions.puckSpeed(ctx.game.world,puck,puck.data.originalSpeed || 1)
      })
      actions.on('opponentPaddleHit',function(){
        replay = true
      })
    }
    ...
    keys.bind(KEYS_LEFT,function(){ isLeft = 1; },'keydown')
    keys.bind(KEYS_LEFT,function(){ isLeft = 0; },'keyup')
    keys.bind(KEYS_RIGHT,function(){ isRight = 1; },'keydown')
    keys.bind(KEYS_RIGHT,function(){ isRight = 0; },'keyup')
  },
  ...
  leave: function(ctx){
    ...
    keys.unbind(KEYS_LEFT,'keyup')
    keys.unbind(KEYS_LEFT,'keydown')
    keys.unbind(KEYS_RIGHT,'keyup')
    keys.unbind(KEYS_RIGHT,'keydown')
