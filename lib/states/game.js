...
exports.Information = {
  enter: function(ctx){
    ...
    if( lock.available && $("#pointer-lock").hasClass("enabled") ) {
      var game = ctx.game;
      var world = game.world;
      //assign pointer to button
      world.pointer = lock(this.play[0]);
    }
...
exports.Start = {
  ...
  enter: function(ctx){
    ...
    if( ctx.game.world.pointer) {
      mouse.off('move');

      ctx.game.world.pointer.on('move',function(dx,dy){
        ctx.game.emit('input',World.MOVE,ctx.game.world.me.paddle,dx * settings.data.mouseSensitivity)
      })
      // fallback to mouse move in case
      // it's denied
      ctx.game.world.pointer.on('error',function(){
        ctx.game.world.pointer = null;
        mouse.on('move',function(dx,dy,dt){
          ctx.game.emit('input',World.MOVE,ctx.game.world.me.paddle,dx * settings.data.mouseSensitivity)
        })
      })

      ctx.game.world.pointer.on('release', function(){
        mouse.on('move',function(dx,dy,dt){
          ctx.game.emit('input',World.MOVE,ctx.game.world.me.paddle,dx * settings.data.mouseSensitivity)
        }) 
      })
...
function startGame(game,local,withPuck){
  ...
  // reset the game
  // TODO can we keep the score? at least until game over...
  game.reset();
