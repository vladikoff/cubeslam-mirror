...
exports.Play = {
  enter: function(ctx){
    ...
    /*dmaf.addEventListener("countdown_tick", function ()

    })

    dmaf.addEventListener("countdown_complete", function ()
...
exports.Next = {
  ...
  enter: function(ctx){
    ...
    // multiplayer + round over = game over!
    if( ctx.multiplayer && roundOver ){
      console.log('multiplayer game over')
      world.over = true;

      if( winner == world.me ) dmaf.tell('user_won_match')
      else dmaf.tell('user_lost_match')

      return see('/game/over')

    // singleplayer + round over + me winner = level up!
    } else if( !ctx.multiplayer && roundOver && winner === world.opponent ){
    ...
    // singleplayer + round over + me winner = level up!
    } else if( !ctx.multiplayer && roundOver && winner === world.me ){
