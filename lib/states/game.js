...
exports.Next = {
  ...
  enter: function(ctx){
    ...
    if( ctx.multiplayer ){
      ...
      $('.state.game-prompt-over .over .win .loose').hide();

      if( winner === world.me ){
        dmaf.tell('user_won_match')
        $('.state.game-prompt-over .win').show();
      } else {
        dmaf.tell('user_lost_match')
        $('.state.game-prompt-over .loose').show();
      }
...
exports.Over = {
  enter: function(ctx){
    ...
    function restart(){
      ...
      console.log('restarted level ' + (ctx.game.world.level.index+1) + ' ' + (tracking.replayClicks++) + ' times')
      _gaq.push(['_trackEvent', (ctx.multiplayer)?'2p':'1p', 'restarted level ' + (ctx.game.world.level.index+1) + ', ' + (tracking.replayClicks++) + ' times' ]);
