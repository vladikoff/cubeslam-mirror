    ...
    if( ctx.multiplayer ){
      ...
      game.on('leave frame',function(world){
        ...
        if( replay ){
          ...
          inputs.network.replay(ctx.sync.world,world,6) // send rate, 15hz = 6 frames
          replay = false;
          freezePaddle = false;
      ...
      actions.on('replay',function(){ replay = true; })
      ...
      function hitOrMiss(x,f){
        console.log('received hit or miss. opponent paddle will be at x: %s',x)
        ...
        // reset the original speed of the puck now that we already positioned the
        // paddle correctly
        // actions.puckSpeed(ctx.game.world,puck,ctx.sync.world.pusk)
      }
    }
