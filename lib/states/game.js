...
exports.Setup = {
  enter: function(ctx){
    ...
    if( ctx.multiplayer ){
      ...
      var renderer = new Renderer(document.getElementById('canv-db'));
      ctx.networkGame = new Game('sync',renderer);
      inputs.context(ctx);
      inputs.on('message',function(buf){ctx.network.send(buf)});
    }

    // set the active player in the renderer
    ctx.game.renderer.activePlayer(ctx.network.winner ? 1 : 0, ctx.multiplayer)
    ...
    if( ctx.multiplayer ){
      ...
      actions.on('round over',this.roundOver) // TODO
...
exports.Next = {
  ...
  enter: function(ctx){
    ...
    if( ctx.multiplayer ){
      ...
      // copy the scores to networkGame
      // so the replays won't break
      var sync = ctx.networkGame.world;
  ...
  leave: function(ctx,next){
    // updating the scores
    // TODO maybe this should not be handled by game.reset()
    ctx.game.reset()
    ctx.networkGame.reset()
    updateScores(ctx.game.world)
