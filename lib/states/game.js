...
exports.Over = {
  enter: function(ctx){
    ...
    function restart(){
      ...
      if(ctx.multiplayer){
        // reset the sync scores manually
        // so they won't be copied back on
        // replay.
        var sync = ctx.sync.world;
        sync.players.a.score = 0;
        sync.players.b.score = 0;
...
function showExtraIcon(type){
  ...
  var el = $('#extras .icon-'+type).show()
    , y = ($('#extras .visible').length-1) * 52;
    el.css('top', y);
    el.addClass('visible');
  // el[0].style.webkitTransform = 'translateY('+y+'px)'
}
