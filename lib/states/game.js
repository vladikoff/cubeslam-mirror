...
function setupLevels(ctx){
  ...
  // TODO move this into lib/Game.js

  var game = ctx.game;

  // the puppeteer takes care of the levels
  ctx.puppeteer = new Puppeteer(game.actions)
  ctx.editor = new Editor(ctx.puppeteer,game.actions)
  ...
  ctx.puppeteer.add(levels.level1);
  ctx.puppeteer.add(levels.level2);
  ctx.puppeteer.add(levels.level3);
  ctx.puppeteer.add(levels.level4);
  ctx.puppeteer.add(levels.level5);
  ctx.puppeteer.add(levels.level6);
  ctx.puppeteer.add(levels.level7);
  ctx.puppeteer.add(levels.level8);

  // let the puppeteer listen to updates
  game.on('update',ctx.puppeteer.update.bind(ctx.puppeteer))
...
function createGame(game,local){
...
function networkInput(ctx){
  ...
  network.on('message',function(buf){
    ...
    // `world.frame == frame`
    if( !ctx.networkGame.world.paused ){
    ...
    while(data.offset < buf.byteLength){
      ...
      switch(type){
        case World.ACK:
          ...
          return; // don't send an ACK on an ACK
        case World.MOVE:
