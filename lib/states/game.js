...
exports.Setup = {
  enter: function(ctx){
    ...
    actions.on('round over',function(world){
      console.log('ROUND HOUSE!',world.name,world.frame,ctx.multiplayer)
    ...
    actions.on('pause',function(world){
      console.log('GAME PAUSE!',world.name,world.frame,ctx.multiplayer)
      if( !ctx.multiplayer || world.name == 'sync' ){
    ...
    actions.on('play',function(world){
      console.log('GAME PLAY!',world.name,world.frame,ctx.multiplayer)
      if( !ctx.multiplayer || world.name == 'sync' ){
    ...
    actions.on('resume',function(world){
      console.log('GAME RESUME!',world.name,world.frame,ctx.multiplayer)
      if( !ctx.multiplayer || world.name == 'sync' ){
    ...
    actions.on('start',function(world){
      console.log('GAME START!',world.name,world.frame,ctx.multiplayer)
      if( !ctx.multiplayer || world.name == 'sync' ){
  ...
  leave: function(ctx){
    ...
    keys.unbind(['right','up','d'],'keyup')
    keys.unbind(['right','up','d'],'keydown')
    keys.unbind(['left','down','a'],'keyup')
    keys.unbind(['left','down','a'],'keydown')
...
exports.Start = {
  ...
  enter: function(ctx){
    ...
    if( ctx.game.world.level.index != tracking.currentLevel ) {
      ...
      if( level > 1 ) {
         _gaq.push(['_trackEvent', (ctx.multiplayer)?'2p':'1p', 'level ' + (level-1) + ' completed', undefined,levelTime ]);
      }
...
exports.Play = {
  ...
  leave: function(ctx){
    ...
    ctx.network.winner && ctx.network.sync.update(10)
  },
...
exports.Pause = {
  enter: function(ctx){
    ...
    this.timeout = setTimeout(function(){
      ...
      $('button.play', ctx.el).on('click',resume)
    }, 1000);
...
exports.Next = {
  ...
  enter: function(ctx){
    ...
    // death ball!
    var wasDeathBall = world.state === World.DEATH_BALL;
    ...
    var gameOver = world.me.score >= maxBalls || world.opponent.score >= maxBalls;
    var winner = world.me.score > world.opponent.score ? world.me : world.opponent;

    // game is automatically over when deathballed
    if( wasDeathBall ){
      //gameOver = true;
...
function startGame(game,local,preview){
  ...
  world.multiplayer = ctx.multiplayer;
