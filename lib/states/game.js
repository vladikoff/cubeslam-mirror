...
exports.Setup = {
  ...
  leave: function(ctx){
    ...
    ctx.editor = null
     ...
     //      (so we don't remove others callbacks)
    ctx.game.off('frame')
    ctx.game.off('update')

    inputs.unbind('right','keyup')
    inputs.unbind('right','keydown')
    inputs.unbind('left','keyup')
    inputs.unbind('left','keydown')
    inputs.unbind('up')
  }
}
...
exports.Start = {
  ...
  enter: function(ctx){
    ...
    // multiplayer
      ...
      //   // then plays back to where it started re-adding the rewound inputs.
...
function setupControls(ctx){
  var game = ctx.game
    ...
    , dx = 0
    , force = 10000
    , paddle = world.paddles.get(world.me.paddle);

  // updates on window resize
  var ww = window.innerWidth
    , wh = window.innerHeight;
  $(window).on('resize',function(){ ww = window.innerWidth; wh = window.innerHeight})

  var lastTime = Date.now()
    , lastFrame = 0;
  game.on('frame',function(world){
    if( world.paused || lastFrame === world.frame ) return
    isLeft && actions.paddlePush(world.me.paddle, -force, 0)
    isRight && actions.paddlePush(world.me.paddle, force, 0)

    if( isMoving ){
      isMoving = false;
      var p = paddle.current[0]-settings.data.arenaWidth/2
        , m = renderer.getWorldCoordinate(mx*2-1,my*-2+1)
      dx = m - p;
    } else {
      dx *= .3
    }
    ...
    // nice and quick (but not using any forces)
    // if( dx && lastFrame != world.frame ){
    //   paddle.previous[0] = paddle.current[0] += dx;
    //   var now = Date.now()
    //   console.log(paddle.current[0],(now-lastTime),world.frame)
    //   lastTime = now;
    //   lastFrame = world.frame;
    // }

    // using forces (but very bouncy when multiplier is large)
    // and seems to behave weird when playing?
    if( dx ){
      actions.paddlePush(world.me.paddle, dx * 100, 0)
    }
    lastFrame = world.frame;
  })
  ...
  inputs.bind('up',function(){ createBullet(world,actions) })
  $(document).on('mousemove',function(e){ isMoving = true; mx = e.pageX / ww; my = e.pageY / wh })
}
