...
exports.Round = {
  ...
  //css transition
  enter: function(ctx){
    if(!ctx.el.data('levelUp'))
      ctx.el.fadeOut(0).fadeIn(200);
    ctx.el.data('levelUp', false);
    $('h2', ctx.el)
      .fadeTo(0, 0)
      .fadeTo(300, 1)
      .delay(600)
...
exports.Start = {
  ...
  leave: function(ctx){
    ctx.renderer.changeView("play");
    $('#start-prompt').data('gameStart', true);
  }
...
exports.Play = {
  enter: function(ctx){
    ...
    if($('#start-prompt').data('gameStart'))
      $('#start-prompt').fadeIn(0).data('gameStart', false);
    else
      $('#start-prompt').fadeOut(0).fadeIn(200);

    $('#start-prompt h2')
      .fadeTo(0,0)
      .fadeIn(300, 1)
    $('#start-prompt').delay(700).fadeOut({
      duration: 300,
      complete: function(){
