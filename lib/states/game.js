  ...
  enter: function(ctx){
    ...
    if( ctx.multiplayer ){
      ...
      inputs.network.on('move',function(id,x){
        if( !freezePaddle && id === ctx.game.world.opponent.paddle ){
          actions.movePaddle(ctx.game.world,id,x)
        }
      })
      inputs.network.on('hit',hitOrMiss)
      inputs.network.on('miss',hitOrMiss)
      actions.on('opponentPaddleHit',function(){ replay = true })
      actions.on('opponentShieldHit',function(){ replay = true })
      ctx.network.on('change latency',function(latency){
        ctx.game.world.latency = latency;
      })
      ctx.game.world.latency = ctx.latency;

      function hitOrMiss(x,f){
        console.log('received hit or miss %s. opponent paddle will be hit at x: %s',f,x,ctx.game.world.frame)
        ...
        // lock the paddle position now until the hit or miss happens
        // so it will reflect/steer correctly
        freezePaddle = true;
