...
exports.Setup = {
  enter: function(ctx){
    ...
    actions.on('showExtra', function(id){
      showExtraIcon(id);
    })
    actions.on('removeExtra', function(id){
      hideExtraIcon(id);
    })
    actions.on('activateExtra', function(id){
      activateExtraIcon(id);
    })
...
exports.Wait = {
  ...
  leave: function(ctx,next){
    ...
    // TODO make sure this is correct? both will have
    //      the same latency so shouldn't only the
    //      non-initiator (!winner) wait a bit extra?
    var offset = ctx.latency || 0;
    clearTimeout(this.timeout);
    this.timeout = setTimeout(next, 1000 - offset);
  },

  cleanupt: function(ctx){
    clearTimeout(this.timeout);
  }
}
...
exports.Start = {
  ...
  enter: function(ctx){
    ...
    if( ctx.sync ){
      // TODO sometimes the multiplayer scores
      //      seems to get out of sync. is this
      //      the culprit?
...
exports.Next = {
  ...
  enter: function(ctx){
    ...
    // round is automatically over when death balled
    if( world.state === World.DEATH_BALL ){
      roundOver = true;
      // TODO winner? whoever didn't take the death ball?
    }
    ...
    // singleplayer
      ...
      // next round!
        ...
        ctx.game.puppeteer.goto(ctx.game.puppeteer.level);
...
exports.Over = {
  enter: function(ctx){
    ...
    if(!ctx.multiplayer){
      ...
      $('#me-levels i').text(ctx.sync.world.me.score);
      $('#opponent-levels i').text(ctx.sync.world.opponent.score);
    }
    ...
    function restart(){
      ...
      //restart at same level, do we need this?
      ctx.game.puppeteer.goto(ctx.game.puppeteer.level);
      $('#scores').fadeIn(500);
...
function setupLevels(ctx,game){
  ...
  if( ctx.onchangelevel )
    game.puppeteer.off('change',ctx.onchangelevel)

  ctx.onchangelevel = function(level){
  ...
  game.puppeteer.on('change',ctx.onchangelevel)
...
function hideExtraIcon(type){
  type = type.replace(' ', '-')
  var el = $('#extras .icon-'+type).removeClass('visible active');
...
function activateExtraIcon(type){
  type = type.replace(' ', '-')
  var el = $('#extras .icon-'+type).addClass('visible active');
}
