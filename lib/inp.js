...
function interpolate(before, after){
  ...
  // skip if < X frames or > Y frames
  if( frames > 12 ){
    console.log('skipping interpolation (too big jump: %s)',frames)
    return;
  }

  if( frames < 1 ){
    console.log('skipping interpolation (no frames: %s)',frames)
    return;
  }
  ...
  for(var i=0; i<after.pucks.length; i++){
    ...
    if( before.pucks.has(a.index) ){
      ...
      var dc = vec.sub(a.current,b.current)
      var dp = vec.sub(a.previous,b.previous)

      // only interpolate it difference is larger
      // than 5 (TODO setting?)
      if( vec.len(dc) > 5 ){
        i.current = vec.sdiv(dc,frames,dc);
        i.previous = vec.sdiv(dp,frames,dp);
        i.frames = frames;

        // no need to free, it will be freed when done
        // console.log('adding interpolation to %s c:%s p:%s f:%s',b.index,i.current.join(','),i.previous.join(','),i.frames)
        // console.log(' before c:%s p:%s',b.current.join(','),b.previous.join(','))
        // console.log(' after  c:%s p:%s',a.current.join(','),a.previous.join(','))
