...
exports.onmessage = function(buf){
  ...
  for(var offset=0; offset<arr.byteLength;){
    var len = arr[offset++];
    var msg = arr.subarray(offset,len);
    offset += len;
...
function enqueue(queue,frame,input){
  ...
  if( frame < last ){
    if( frame == 0 ){
      ...
      alert('received an input too early. determinism is not guaranteed.')
...
function dequeue(queue,world){
  ...
  if( queue[0] < world.frame ){
    alert('cannot pass the first frame in queue. determinism is not guaranteed.')
...
function verify(a,b){
  debug('verify')
  if( a.code() !== b.code() ){
    alert('hash codes does not match after copy. determinism is not guaranteed.')
...
function verifySequence(seq){
  if( Math.abs(lastSeq-seq) > 1 ){
    ...
    if( seq == 0 || lastSeq == 0 ){
      ...
      alert('dropped packets. determinism is not guaranteed.')
