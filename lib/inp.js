...
exports.onmessage = function(buf){
  ...
  // respond with an ACK to let the others know
  // we got it (and so their sync game can get
  // to the same point)
  // but only if the message contained other input
  // than ACKs or it would end up in a dumb loop.
  if( !onlyACKs ){
...
setInterval(function(){
  if( sendlen ){
    ...
    // emits a reminder for the DataChannel
    // (or actually the NetChannel) to empty
    // it's buffer.
    // exports.emit('flush');

    // send an ack every 100ms to keep them
    // at least sort of up to date even if
    // no one moves...
    // moved this from the process loop because
    // it is frozen in case the "games are too
    // far apart"
    if( !paused && sync.world.state == World.PLAYING ){
      exports.record(exports.ACK);
    }
  }
},1000/30)
