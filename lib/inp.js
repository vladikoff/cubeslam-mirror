...
exports.record = function(){
  debug('record %s', replaying ? '(replay)' : forwarding ? '(forwarding)' : '', str(arguments))
...
exports.process = function(world){
  ...
  for(var i=0; i<buffer.length; i++){
    // skip ACKs they only need to be sent
    // over the network anyway
    if( buffer[i][0] == exports.ACK ){
      continue;
    }
...
exports.onmessage = function(buf){
  ...
  for(var offset=2; offset<arr.byteLength;){
    ...
    for(var i=1; i<inputs.length; i++){
      // skip acks. we use the `ack` variable
      // to set how far to forward anyway and
      // that one is set even if the input is
      // not enqueued.
      if( inputs[i][0] != exports.ACK ){
        enqueue(queues.net,frame,inputs[i])
      }
    }

    if( ack > frame ){
      console.warn('frame is too late?',ack,frame)
      continue;
    }
...
function forward(){
  ...
  debug('forward from %s -> %s %s',a,b,b === ack ? '(ack)' : '(game)')

  // only time when a actually stepping a
  // frame to remove some noise
  if( b - a > 10 ){
    console.time('forward from '+a+' -> '+b)
  }
  ...
  for(var i=a; i<b; i++){
    ...
    // state has changed. make sure we
    // replay to/from this state
    if( sync.world.state !== state ){
      console.log('state was changed during forward at frame %s. skipping the rest of the frames.',sync.world.frame)

      // revert (only copy the sync state) instead of replay
      revert = true;
      break;
    }
  }
  ...
  if( b - a > 10 ){
    console.timeEnd('forward from '+a+' -> '+b)
  }
...
function replay(){
  debug('replay %s > %s %s',sync.world.frame,game.world.frame,revert ? '(revert)' : '')
  var a = sync.world.frame;
  var b = game.world.frame;
  // console.time('replay from '+a+' -> '+b)

  // console.time('replay copy')
  ...
  // when revert is true just copy the
  // sync world and we're done...
  if( revert ){
    revert = false;
    queues.loc.length = 0;
    queues.net.length = 0;
  ...
  // console.time('replay execute and update')
  ...
  // }
  replaying = false;
  // console.timeEnd('replay execute and update')

  // add interpolation between the temp
  // and the replayed world.
  // console.time('replay interpolate')
  // interpolate(temp,game.world)
  // console.timeEnd('replay interpolate')

  debug('replay end')
  // console.timeEnd('replay from '+a+' -> '+b)
}
...
function interpolate(before, after){
  ...
  for(var i=0; i<after.pucks.length; i++){
    ...
    if( before.pucks.has(a.index) ){
      var b = before.pucks.get(a.index);
      if( !b ){
        console.warn('pucks has(b) !== get(b) ?! something must be wrong with Stash#del()',a.index);
        debugger;
        continue;
      }
      interpolateBody(a,b,frames)
    }
  }
  ...
  for(var i=0; i<after.paddles.length; i++){
    ...
    if( before.paddles.has(a.index) ){
      var b = before.paddles.get(a.index);
      if( !b ){
        console.warn('paddles has(b) !== get(b) ?! something must be wrong with Stash#del()',a.index);
        debugger;
        continue;
      }
      interpolateBody(a,b,frames)
...
// integrate and check collisions of
// pucks, paddles and bullets over `n`
// frames.
function extrapolate(game,n,loc,net){
  var world = game.world
    , timestep = 1/60; // TODO don't hard code!

  if( n > 10 ){
    console.warn('extrapolating more than 10 frames: %s',n)
  ...
  for(var i=0; i<n; i++){
    ...
    // only update the physics, no need for recording/processing
    // inputs and ai etc. they should only be ran by sync and/or
    // has already been ran ones (this is a replay after all)
    physics.update(world,timestep)

    // if( world.state == 'playing' ){
    //   for(var j=0; j<world.pucks.length;j++){
    //     physics.updateBody(world,tsq,world.pucks.values[j])
    //   }
    //   for(var j=0; j<world.paddles.length;j++){
    //     physics.updateBody(world,tsq,world.paddles.values[j])
    //   }
    //   for(var j=0; j<world.bullets.length;j++){
    //     physics.updateBody(world,tsq,world.bullets.values[j])
    //   }
    //   world.frame++;
    // }
  }
