...
function Editor(puppeteer,actions){
...
Editor.prototype = {
  ...
  save: function(){
    ...
    $('<textarea style="width:500px;height:500px;">').html(json).appendTo(c.el.find('.body'));
    this.gui.close()
  },
  ...
  setup: function(){
    ...
    this.gui.add(this,'hideUI').name("Hide UI").onChange(function(value){
      $(".state").css("opacity",value?0:1);
    })
  ...
  _setupGUI: function(){
    ...
    $("#settingsLevelsGUI").find(".close-button:last").click( function(){
     ...
     setTimeout(function(){
      $("#settingsLevelsGUI").append(this.gui.domElement);
    }.bind(this),2000)
...
Level.prototype = {
  ...
  add: function(event,i){
    var e = new Event(this,event,availableActions(this.editor.actions));
...
function Event(level,obj,actions){
  obj = obj || {}
  this.level = level;
  this.event = obj;
  ...
  this.folder.add(this.event,'action').options(actions).onChange(this.change.bind(this))
  ...
  change: function(action){
    ...
    switch(action){
      ...
      case 'forceCreate':
        ...
        p.push(f.add(this.event.params,'1').min( settings.data.unitSize*.5).max(settings.data.arenaWidth-settings.data.unitSize*.5).step(settings.data.unitSize*.5).name('X').onChange( function(){onMarkerPosition(this.event.params)}.bind(this) ))
        p.push(f.add(this.event.params,'2').min( settings.data.unitSize*.5).max(settings.data.arenaWidth).step(settings.data.unitSize*.5).name('Y').onChange( function(){onMarkerPosition(this.event.params)}.bind(this) ))
      ...
      case 'extraCreate':
        ...
        p.push(f.add(this.event.params,'1').min(settings.data.unitSize*.5).max(settings.data.arenaWidth-settings.data.unitSize*.5).name('X').step(settings.data.unitSize*.5).onChange( function(){onMarkerPosition(this.event.params)}.bind(this) ))
        p.push(f.add(this.event.params,'2').min(settings.data.unitSize*.5).max(settings.data.arenaHeight-settings.data.unitSize*.5).name('Y').step(settings.data.unitSize*.5).onChange( function(){onMarkerPosition(this.event.params)}.bind(this) ))
        break;
...
function availableActions(actions){
  var all = Object.keys(actions).filter(function(a){
    if( a.indexOf('puck') == 0 )
      return false;
