var debug = require('debug').enable('game actions ai')
  , Game = require('./game')
  , rtc = require('./rtc')
...
var NetworkState = {
  ...
  create: function(){
    ...
    this.remote.on('open',function(e){
      if( this.channel ){
        console.log('REMOTE OPEN (ignored)')
        return;
      }

      console.log('REMOTE OPEN (creating an invitedata channel)')
  ...
  create: function() {
    ...
    $("#scanButton").bind("click",function(){
      ...
      this.timeout = setTimeout(function(){
        if (this.game.hasState(GameStartState)) {
          this.game.switchState(this.nextState || PlayState);
        } else {
...
var GameStartState = {
  ...
  create: function() {
    ...
    this.simulator = new Simulator(Math.round(Math.random()*100));
...
var GameOverState = {
  ...
  create: function() {
    ...
    $('#gameOverDialog .play').bind('click', function() {
      this.game.switchState(GameStartState);
      $('#headBar').removeClass('playingmulti'); // We will always start into single player mode, so the headBar should be able to display the "friend is waiting"-stuff again.
      return false;
    }.bind(this));
