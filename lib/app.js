...
var MainMenuState = {
  ...
  create: function(){
    ...
    document.getElementById('otherplay') && document.getElementById('otherplay').addEventListener('click', function() {
...
var PaddleSetupState = {
  ...
  create: function() {
    ...
    // This state requires the camera to have been activated.
    /*
    if (!camera.active) {
    ...
    $("#scanButton").one("click",function(){
      ...
      setTimeout(function(){
        if (NetworkState.connect) {
            this.game.switchState(MultiState);
          } else {
            this.game.switchState(SingleState);
          }
      }.bind(this),1000)
...
var NetworkState = {
  ...
  create: function(){
    ...
    var remote = rtc.connect(channelToken, conferenceRoom, getCookie("clientId"))
...
var SingleState = {
  ...
  create: function(){
    ...
    // This state requires the paddle to be set up:
    /*
    if (!PaddleSetupState.scanned) {
      this.game.switchState(PaddleSetupState);
      return;
    }
    */
