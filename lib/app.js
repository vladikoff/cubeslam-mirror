...
module.exports = function main(ctx){
  ...
  var log = require('./support/tailstream')(ctx.room);
  log.prefix = "\033["+color(ctx.user)+"m"+ctx.user+"\033[0m ";
  log.filter('rtc:')
  log.filter(function(msg){
    return msg.indexOf('icechange') === -1
  })
  log.transform(function(msg){
    var i = msg.indexOf('{ sdp:');
    if( i == -1 ){
      return; // ignore
    }
