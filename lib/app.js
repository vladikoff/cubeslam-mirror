    ...
    assertSim = function(){
      ...
      for(var f=0; f < this.host.forces.length; f++ ){
        if(this.host.forces[f].toString() !== this.guest.forces[f].toString())
          console.warn('forces does not match: '+this.host.forces[f].toString()+' !== '+this.guest.forces[f].toString())
      }
      ...
      for(var p=0; p < this.host.pointMasses.length; p++ ){
        if(this.host.pointMasses[p].current.toString() !== this.guest.pointMasses[p].current.toString())
          console.warn('pointMass current position does not match:'+this.host.pointMasses[p].current.toString()+' !== '+this.guest.pointMasses[p].current.toString())
        if(this.host.pointMasses[p].previous.toString() !== this.guest.pointMasses[p].previous.toString())
          console.warn('pointMass previous position does not match:'+this.host.pointMasses[p].previous.toString()+' !== '+this.guest.pointMasses[p].previous.toString())
        if(this.host.pointMasses[p].velocity.toString() !== this.guest.pointMasses[p].velocity.toString())
          console.warn('pointMass velocity does not match:'+this.host.pointMasses[p].velocity.toString()+' !== '+this.guest.pointMasses[p].velocity.toString())
        if(this.host.pointMasses[p].acceleration.toString() !== this.guest.pointMasses[p].acceleration.toString())
          console.warn('pointMass acceleration does not match:'+this.host.pointMasses[p].acceleration.toString()+' !== '+this.guest.pointMasses[p].acceleration.toString())
      }
    }.bind(this)
  },
  destroy: function(){
