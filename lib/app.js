var debug = require('debug').enable('game rtc:* actions')
  , Game = require('./game')
  , rtc = require('./rtc')
  ...
  create: function() {
    ...
    $('.soundSwitch').click(function() {
      if ($(this).hasClass('on')) {
        ...
        if( settings.data.playSoundtrack ) audio.stop("soundtrack");
        world.sounds = false;
      } else {
        $.cookie('sound', 'on');
        $(this).removeClass('off').addClass('on');
        //settings.data.playSoundtrack = true;
        if( settings.data.playSoundtrack ) audio.play("soundtrack");
        world.sounds = true;
      }
      return(false);
    });
...
var NetworkState = {
  ...
  create: function(){
    ...
    // Truly bad-ass solution:
    // Eftersom att reliable i DataChannels inte riktigt rullar, samt eftersom att vi inte riktigt vet när anslutningar bryts
    // och när det är en glitch mellan anslutningar då vi råkar sända data, så gör vi så här. TODO FIXME ETC

    setInterval(function() {
      var stateName = NetworkState.game.states[NetworkState.game.states.length - 1].name;
      if (stateName != 'Pause') {
        this.channel && this.channel.send(JSON.stringify({state: stateName}));
      }
    }.bind(this), 1000);
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        this.remote.on('participantsUpdated', function() {
          participants = roomParticipants.split("|");
...
var PaddleSetupState = {
  ...
  controls: function(inputs){
    if (inputs.pressed('next')) {
      ...
      //don't save result, just temporary
