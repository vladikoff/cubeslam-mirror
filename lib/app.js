var debug = require('debug').enable('game rtc:* time-sync')
  , Game = require('./game')
  , rtc = require('./rtc')
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        this.remote.on('open',function(){
          ...
          //removed this line so the popup can be triggered always, but still need to check other glitches with this
          //if( game.isState(InviteFriendState) || game.isState(WaitingFriendState) ){

          //TODO better way of checking if this is the first time or after accepted cameras
          if( !camera.active && !game.isState(WebcamActivationState) ){
            if( world.host )
              game.switchState(FriendArrivedState)
            else {
              if( !game.isState(MainMenuState) ){
        ...
        this.remote.on('datachannel',function(e){
          ...
          if( e.channel.label.indexOf('timesync') == 0 ){
            console.warn('time sync channel!',e.channel)
            TimeSyncState.channel = e.channel;
          } else {
            console.warn('game start channel!',e.channel)
            GameStartState.channel = e.channel;
          }
        })
        ...
        var channelId = 0;
        this.remote.on('reconnect',function(connection){
          console.log('reconnecting',world.host,connection)
          // TODO use reliable instead (only unreliable currently supported...)
          if( world.host ){
            var id = channelId++;
            connection.createDataChannel('timesync'+id,{reliable:false});
            connection.createDataChannel('gamedata'+id,{reliable:false});
            console.warn('created new data channels',id)
          }
        })
...
var WebcamActivationState = {
  ...
  cameraOn: function( stream ) {
    ...
    WebcamActivationState.game.switchState( WaitForFriendCameraState);
  },
...
var TimeSyncState = {
  ...
  channel: null, // should be set by NetworkState before video has been established (because of a bug in chrome with creating a datachannel before adding streams...)
  latency: undefined,
  ...
  create: function(){
    ...
    // TODO sometimes this.channel is undefined. I think it happens
    //      when webcam has been allowed before the other player has
    //      clicked "play friend".

    // Use time-sync to sync the time between the peers
    // before going to the PlayState
    TimeSync(this.channel)
      .on('timeout',function(){
      ...
      .on('done',function(host){
        ...
        TimeSyncState.channel.close()
        TimeSyncState.channel = null;
      })
      .start(world.host)
...
var GameInstructionsState = {
  ...
  create: function() {
    ...
    this.playButton = gameInstructions.find(".play").show()
  ...
  destroy: function() {
    ...
    this.game.inputs.unbind('space', 'next');
...
var GameStartState = {
  ...
  puppeteer: null,
  simulator: null,
  channel: null,    // should be set by on 'datachannel' in NetworkState
...
var PauseState = {
  ...
  create: function() {
    ...
    if( GameStartState.channel ){
      $('.state.Pause .playFriend').hide();
    }
    else{

      $('.state.Pause .playFriend').show().on('click', function(){

        //TODO this can be broken?
        //TODO fix the back button from InviteFriendState to here?
        this.game.switchState(InviteFriendState)
...
var OpponentLeftState = {
  ...
  create: function() {
    ...
    $('.state.OpponentLeft .play').each(function() {
     ...
     $('.state.OpponentLeft .playFriend').on('click', function(){
