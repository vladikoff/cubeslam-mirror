...
var RootState = {
  ...
  controls: function(inputs){
    ...
    if( inputs.pressed('toggleinput') ){
      switch(settings.data.inputType){
        case 'mouse': settings.data.inputType = 'keyboard'; break;
...
var NetworkState = {
  ...
  create: function(){
    ...
    var Synchronizing = {
      ...
      redirect: function() {
        if (NetworkState.opponentGameState == 'WebcamActivation'
         || NetworkState.opponentGameState == 'PaddleSetup') {
    ...
    var OpponentIsReady = {
      ...
      letOpponentIn: function() {
        ...
        while(NetworkState.game.hasState(GameStartState)
           && !NetworkState.game.isState(GameStartState))
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        this.remote.on('promoted',function(){
          ...
          if (NetworkState.game.hasState(WaitForHostState)
           && !wasHost
           && GameStartState.channel == null) {
            game.popState(); // huh?
            game.popState(); // what?
            game.switchState(OpponentLeftState);
          }
        })
...
var MainMenuState = {
  ...
  create: function(){
    ...
    $('#mainmenu .playFriend').bind('click', function() {
      this.game.switchState(this.nextState || WebcamActivationState);
    }.bind(this));

    $('#mainmenu .playComputer').bind('click', function() {
      this.game.switchState(this.nextState || GameInstructionsState );
    }.bind(this));
  ...
  onresize: function(){
    /*var f = 0.2 + Math.min(window.innerWidth/3000,0.8);
...
var WebcamActivationState = {
  ...
  cameraOn: function( stream ) {
    ...
    videoInput.width = 320//INPUT_WIDTH*INPUT_SCALE;
    videoInput.height = 240//INPUT_HEIGHT*INPUT_SCALE;
...
var GameStartState = {
  ...
  create: function() {
    ...
    this.puppeteer.on('update',function(world){
      // TODO specific logic...

      // TODO is the game over yet?
      // this.emit('game over')

      // TODO should we level up?
      // this.up()

      // TODO should we level down?
      // this.down()
    })
...
var PlayState = {
  ...
  create: function(){
    ...
    this.timeout = setTimeout(function(){ countdown(3) }, 0 - offset);
  },
