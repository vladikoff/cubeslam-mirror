...
var InviteFriendState = {
  ...
  create: function(){
    createState(this)
    $('.InviteFriend .backToMainMenu').on('click',function(){
  ...
  controls: function(inputs) {
    if (inputs.pressed('back')) {
      $('.InviteFriend .backToMainMenu').click();
    }
  },
  render: function(alpha){
    world.renderer.render(world, alpha)
  ...
  destroy: function(){
    destroyState(this)
    this.game.inputs.unbind('space');
    $('.InviteFriend .backToMainMenu').off('click');
  }
}
...
var GameStartState = {
  ...
  create: function() {
    ...
    if( levelIndex != -1){
      var level = int(window.location.href.charAt(levelIndex+6));
