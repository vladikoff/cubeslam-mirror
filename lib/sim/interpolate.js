...
function interpolateBody(a,b,f){
  ...
  if( i.frames ){
    ...
    // i.offset = null
    // i.frames = null
    console.log('updating interpolation of %s because it already has one')
    return;
  }
  ...
  if( dist < minDist*minDist ){
    console.log('skipping too short interpolation for %s (dist: %s)',a.index,Math.sqrt(dist))
  } else if( dist > maxDist*maxDist ){
    console.log('skipping too long interpolation for %s (dist: %s)',a.index,Math.sqrt(dist))
  } else {
