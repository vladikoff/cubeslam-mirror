...
exports.colliding = function(world,a){
  for(var j=0; j < world.bodies.length; j++){
    ...
    //fix for preventing obstacles to coolide with each other when spawning
...
function Collide(world, a, b, c){
  ...
  switch(a.id){
    case 'puck':  Collide.puck(world,a,b,c); break;
    case 'shield':  Collide.shield(world,a,b,c); break;
    case 'bullet':  Collide.bullet(world,a,b,c); break;
    case 'paddle':  Collide.paddle(world,a,b,c); break;
  }
}
...
Collide.puck = function(world, p, q, c){
  debug.collide('puck',p.index,BodyFlags.toString(p._flags),' > ',q.id,q.index,BodyFlags.toString(q._flags));

  // skip if it has been removed
  // (like if it hit the player already in onbounds)
  if( p.removed ){
    debug.collide('  (ignored because removed)')
    return;
  }
  ...
  // hit a shield
  } else if( world.shields.has(q.index) ){
    actions.hitShield(world,q,p)
...
Collide.paddle = function(world,s,p,c){
  debug.collide('paddle',p.index,s.index,c)
  //paddle is always hit colliding with bullet
  if(p.id=='bullet') {
    Collide.bullet(world,p,s)
  }
  if(p.id=='puck') {
    Collide.puck(world,p,s,c)
  }
}
