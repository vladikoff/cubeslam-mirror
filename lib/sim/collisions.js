...
exports.pointMassToRect = function(pointMass, rect){
  if( !rect.within(pointMass.current) ){
    ...
    if( intersections.length > 1 ){
      ...
      reflect(pointMass, intersection[0])
      pointMass.onbounds(intersection[0])
      return rect.restrain(pointMass.current)

      console.log('RECT got multiple intersections, picked the closest',intersections,intersection.toString(),pointMass.toString())
...
exports.pointMassToPoly = function(pointMass, poly){
  if( poly.within(pointMass.current) ){
    ...
    if( intersections.length > 1 ){
      ...
      console.log('POLY got multiple intersections, picked the closest',intersections,intersection.toString(),pointMass.toString())
