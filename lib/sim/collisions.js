...
exports.pointMassToRect = function(pointMass, rect){
  if( !rect.within(pointMass.current) ){
    ...
    if( intersections.length > 1 ){
      ...
      exports.reflect(pointMass, intersection)
...
exports.pointMassToPoly = function(pointMass, poly){
  if( poly.within(pointMass.current) ){
    ...
    // oncollision should call Collisions.reflect()
    pointMass.oncollision(intersection,poly);
  }
}

/*
 * Attempts to reflect the pointMass based on the angle
 * of attack and intersection normal.
 */
exports.reflect = function(pointMass, intersection){
  // Reflect against surface normal (n)
  // v' = v - 2(v⋅n)n
  // TODO double check this math with http://paulbourke.net/geometry/reflected/
  //      (esp. try using unit vectors and make sure R1 = -Ri)
...
/*
 * Ignores the reflection angle and only bounces off based
 * on the intersection normal.
 */
exports.reflectOnNormal = function(pointMass, intersection){
  var pd = Point.distance(intersection,pointMass.previous)
    , cd = Point.distance(intersection,pointMass.current)
    , n = intersection.normal
    // , p = pointMass.toString();
  pointMass.previous.set(intersection).add(n.x*-pd,n.y*-pd)
  pointMass.current.set(intersection).add(n.x*cd,n.y*cd)

  // console.log('reflected (distance: %s)',Point.distance(pointMass.previous,pointMass.current))
  // console.log(' was: %s',p)
  // console.log(' now: %s',pointMass.toString())
  // console.log(' intersection: %s',intersection.toString())
  // console.log(' normal: %s',n.toString())
  // console.log(' distance intersection->previous: %s',pd)
  // console.log(' distance intersection->current: %s',cd)
  // console.log(' intersection: %s',intersection.toString())
}


// not really correct but feels right
// http://jsfiddle.net/slaskis/7MKuv/5/
exports.ellipticNormal = function(rx,ry,ox,oy,normal){
  // convert to a point within ellipse
  var px = ox*rx
    , py = oy*ry;


  // theta = the angle between the point (or cross section) and the normal
  // this will find the point in the circumference
  // (but not really, see fiddle)
  var theta = normal.y === 1 ? Math.PI*0.5 : Math.PI*1.5
    , x=px+Math.cos(theta)*rx
    , y=py+Math.sin(theta)*ry

  console.log(normal.toString(),theta)

  // this will get the normal at that point
  var theta = Math.atan2(y/ry,x/rx*(ry/rx))
  ...
  var r = Math.sqrt(nx*nx+ny*ny)
    , ux = nx/r
    , uy = ny/r;
  normal.set(ux,uy).fix(4);
  console.log('unit normal (from ellipseNormal)',normal.toString())
}
