...
exports.pointMassToPoly = function(pointMass, poly){
  if( poly.within(pointMass.current) ){
    // Find intersection
    var intersections = intersectsPolyLine(poly, pointMass.previous, pointMass.current);
...
function intersectsRectLine(rect,a1,a2){
  // return intersectsPolyLine(rect,a1,a2)
  var pt
    , results = [];

  pt = intersectsLineLine(rect.topLeft,rect.topRight,a1,a2)
  if( pt ) results.push(pt);
  pt = intersectsLineLine(rect.topRight,rect.bottomRight,a1,a2)
  if( pt ) results.push(pt);
  pt = intersectsLineLine(rect.bottomRight,rect.bottomLeft,a1,a2)
  if( pt ) results.push(pt);
  pt = intersectsLineLine(rect.bottomLeft,rect.topLeft,a1,a2)
  if( pt ) results.push(pt);
...
function intersectsPolyLine(poly,a1,a2){
  ...
  for( var i=0; i < edges.length; i++ )
    if( pt = intersectsLineLine(edges[i].a,edges[i].b,a1,a2) ) 
      results.push(pt);
  return results.length ? results : null;
}
