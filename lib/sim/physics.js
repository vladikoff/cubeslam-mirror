...
// preallocated object to keep the
// collision information
var c = {};

Physics.prototype = {

  updateBody: function(world, tsq, a){
    // check bounds
    ...
    for(var k=0; k <world.bodies.length; k++ ){
      ...
      if( c.willIntersect ){
        if( a.id == "puck" || b.id == "puck" ) {
          ...
          // BOUNCE
          if( BodyFlags.has(puck,BodyFlags.BOUNCE) && BodyFlags.has(other,BodyFlags.BOUNCE) ){
            // console.log("BOUNCE!")
          ...
          console.log('non puck colliding!')
        }
    ...
    for(var j=0; j < world.forces.length; j++){
      if(a.id == "puck"){
        integration.force(world.forces[j],a);
      }
    }

    // move
    integration.body(a,tsq)
  },
      ...
      for(var k=j+1; k <bodies.length; k++ ){
        ...
        if( c.willIntersect ){
          ...
          //only if a puck involved
            ...
            // DYNAMIC (move out of intersection)
            if( !BodyFlags.has(puck,BodyFlags.STATIC) && BodyFlags.has(other,BodyFlags.STATIC) ){
              console.log('%s moving %s out of intersection from %s',world.frame,a.index,b.index)
              console.log(' p',a.previous)
              console.log(' c',a.current)
              console.log(' t',c.minTranslationVector)

              vec.add(a.previous,c.minTranslationVector,a.previous)
              vec.add(a.current,c.minTranslationVector,a.current)

              console.log('moved out of intersection')
              console.log(' p',a.previous)
              console.log(' c',a.current)
              console.log(' t',c.minTranslationVector)
            }
