...
function broadphase(world,mayCollide,wasRemoved){
  for(var i=0; i<world.bodies.length; i++){
    ...
    for(var j=0; j<world.bodies.length; j++){
      ...
      // skip bullet > extra collisions
      if( a.id == 'bullet' && b.id == 'extra' )
        continue;

      // skip extra > bullet collisions
      if( a.id == 'extra' && b.id == 'bullet' )
        continue;

      // skip bullet > puck collisions
      if( a.id == 'bullet' && b.id == 'puck' )
        continue;

      // skip puck > bullet collisions
      if( a.id == 'puck' && b.id == 'bullet' )
        continue;
      ...
      // skip obstacle > extra collisions
      if( a.id == 'obstacle' && b.id == 'extra' )
        continue;

      // skip extra > obstacle collisions
      if( a.id == 'extra' && b.id == 'obstacle' )
        continue;

      // skip extra > extra collisions
      if( a.id == 'extra' && b.id == 'extra' )
        continue;

      // skip extra > shield collisions
      if( a.id == 'extra' && b.id == 'shield' )
        continue;

      // skip shield > extra collisions
      if( a.id == 'shield' && b.id == 'extra' )
        continue;

      // skip obstacle > shield collisions
      if( a.id == 'obstacle' && b.id == 'shield' )
        continue;

      // skip shield > obstacle collisions
      if( a.id == 'shield' && b.id == 'obstacle' )
        continue;

      // skip obstacle > paddle collisions
      if( a.id == 'obstacle' && b.id == 'paddle' )
        continue;

      // skip paddle > obstacle collisions
      if( a.id == 'paddle' && b.id == 'obstacle' )
        continue;

      // skip extra > paddle collisions
      if( a.id == 'extra' && b.id == 'paddle' )
        continue;

      // skip paddle > extra collisions
      if( a.id == 'paddle' && b.id == 'extra' )
        continue;
      ...
      var n = d - al - bl;
      if( d-(ar+br) < 0 ){
        // console.log('%s broadphase says %s:%s and %s:%s may intersect',world.name,a.id,a.index,b.id,b.index)
        // console.log('  a radius: %s',ar)
        // console.log('  b radius: %s',br)
        // console.log('  a velocity: %s',al)
        // console.log('  b velocity: %s',bl)
        // console.log('  distance: %s',d)
        // console.log('  next distance: %s',n)
        // TODO should we check if the pair is in mayCollide first?
        mayCollide.push(a,b);
      // } else if( n-(ar+br) < 0 ){
      //   console.log('%s broadphase says %s:%s and %s:%s may intersect next',world.name,a.id,a.index,b.id,b.index)
      //   console.log('  a radius: %s',ar)
      //   console.log('  b radius: %s',br)
      //   console.log('  distance: %s',d)
      //   mayCollide.push(a,b);
      // } else if( a.index == 9 || b.index == 9 ){
      //   console.log('%s broadphase says %s:%s and %s:%s can NOT intersect',world.name,a.id,a.index,b.id,b.index)
      //   console.log('   distance: %s',d)
      //   console.log('   next distance: %s',n)
      }
    }
