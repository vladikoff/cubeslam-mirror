      ...
      for(var k=j+1; k <bodies.length; k++ ){
        ...
        if( c.willIntersect ){
          ...
          if( a.id == "puck" || b.id == "puck" ) {
            ...
            var coll = (a.id == "puck")?b:a;

            if( BodyFlags.has(puck,BodyFlags.GHOST) && !BodyFlags.has(coll,BodyFlags.STATIC) ){
               //console.log("GHOST COLLISION!")
              // just ignoring for now (maybe even skip oncollision() ?)
              // TODO bounce off of STATIC

            // BOUNCE
            } else if( BodyFlags.has(puck,BodyFlags.BOUNCE) && BodyFlags.has(coll,BodyFlags.BOUNCE) ){
              // console.log("BOUNCE!")

              // DYNAMIC (move out of intersection)
              if( !BodyFlags.has(puck,BodyFlags.STATIC) && BodyFlags.has(coll,BodyFlags.STATIC) ){
                var t = vec.make()
              ...
              if( BodyFlags.has(coll,BodyFlags.STEER) ){
                ...
                var I = vec.norm(puck.velocity)
                  ...
                  , d = (puck.current[0] - coll.current[0])/(coll.aabb[1]-coll.aabb[3]);
            ...
            collisions.oncollision(world,puck,coll,c);
          }
          else {
