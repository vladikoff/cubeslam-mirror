...
exports.body = function(body,tsq){
  ...
  // current + step + diff/frames
  if( i && i.frames ){
    // console.log('applying interpolation to %s over %s frames',body.index,i.frames)
    // console.log(' before c:%s p:%s',c.join(','),p.join(','))

    // x = step + diff/frames
    vec.add(c,i.current,c)
    vec.add(p,i.previous,p)
    ...
    // console.log(' after  c:%s p:%s',c.join(','),p.join(','))
