...
function Inputs(element){
  ...
  tracker.on('userMedia',function(stream,source){console.log('got some userMedia!',stream)})
...
function onkeydown(e){
  ...
  if( !action ) return console.log('ignoring key down',eventCode(e));
  if( !this.down[action] )
...
function onkeyup(e){
  var action = this._bindings[eventCode(e)];
  if( !action ) return console.log('ignoring key up',eventCode(e),this._bindings);
