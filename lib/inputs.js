...
function Inputs(element){
  ...
  tracker.init();
  tracker.on('userMedia',function(stream,source){ this.motion.userMedia = stream; }.bind(this))
  tracker.on('colorUpdate',function(r,g,b){console.log('got some color',r,g,b)})
  tracker.on('videoTexture',function(canvas){ this.motion.videoTexture = canvas; }.bind(this))
