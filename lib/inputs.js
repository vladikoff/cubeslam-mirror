...
Inputs.prototype.copy = function(from){
  copy(from.history,this.history)
  copy(from.frames,this.frames)
  copy(from.current,this.current)
}
 ...
 *   inputs.readFrom(0,100,[ctx.game.inputs,ctx.sync.inputs])
...
Inputs.prototype.record = function(type,a,b,c){
  debug('record',type,a,b)

  switch(type){
    case World.MOVE:
      this.current.push(type,a,b);
      break;
    case World.SHOOT:
