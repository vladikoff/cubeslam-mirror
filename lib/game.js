...
function Game(name,renderer){
  ...
  this.puppeteer.add(levels.level9);
  this.puppeteer.add(levels.level10);
  this.puppeteer.add(levels.level11);
  this.puppeteer.add(levels.level12);
  this.puppeteer.on('change',function(level){
    if( this.ai ){
      this.ai.updateBrain(this.puppeteer.levels[level].ai)
    }
  }.bind(this))
