...
Game.prototype.pushState = function(state){
  ...
  this.emit('stateChange', this.getName(state));
  state.game = this;
  this.currentState = this.getName(state);
  $('body').addClass(this.currentState);
  state.create && state.create();
  $('.state.' + this.getName(state)).addClass('active').removeClass('inactive');
  return this;
}

Game.prototype.popState = function(){
  var state = this.states.pop();
  $('.state.' + this.getName(state)).removeClass('active').addClass('inactive');
  debug('popState',this.getName(state))
  if( state ){
    $('body').removeClass(this.currentState);
...
Game.prototype.switchState = function(state){
  ...
  if( this.isState(state) )
    return this;
  return this.popState().pushState(state);
}
