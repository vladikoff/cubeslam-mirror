...
Game.prototype.pushState = function(state){
  ...
  this.emit('pushState', this.getName(state));
  state.game = this;
...
Game.prototype.popState = function(){
  ...
  if (this.getName(state) == 'Network') console.error('Network removed. WTF.');
  if( state ){
    this.emit('popState', this.getName(state));
...
Game.prototype.switchState = function(state){
  ...
  if( this.hasState(state) ) return this;
  return this.popState().pushState(state);
}

Game.prototype.toString = function(){
  var game = this;
  return this.states.map(function(s){ return game.getName(s) }).join(' > ')
}
