...
Game.prototype.run = function(){
  ...
  function loop(){
    ...
    if( !game.paused ){
      ...
      var updatesEnd = now();
      if( updatesEnd - updatesStart > 5 ){
        console.warn('slow update: %sms',(updatesEnd - updatesStart).toFixed(2))
      }
      ...
      var renderEnd = now();
      if( renderEnd - renderStart > 13 ){
        console.warn('slow render: %sms',(renderEnd - renderStart).toFixed(2))
      }
    }
    ...
    if( frameEnd-frameStart > 16 ){
      console.warn('slow frame:  %sms\n',(frameEnd-frameStart).toFixed(2));
    }
  }
  this.running = true;
  loop();
