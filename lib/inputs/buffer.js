...
exports.build = function(frame,inputs){
  ...
  for(var i=0; i<inputs.length; i++) {
    var input = inputs[i];
    switch(input[0]){
      case types.EMIT: // EMIT
        dat.setInt8(input[0]);    // type
        dat.writeString(input[1]);// id
        dat.setUint16(input[2]);   // x
        break;
      case types.MOVE: // MOVE
      case types.HIT: // HIT
        dat.setInt8(input[0]);    // type
...
exports.parse = function(buf){
  ...
  while(dat.offset < buf.byteLength){
    var input = []
      , type = dat.getInt8();
    switch(type){
      case types.EMIT: // EMIT: type, id, x
        input.push(type,dat.readString(),dat.getUint16())
        break;
      case types.MOVE: // MOVE: type, id, x
      case types.HIT: // HIT: type, id, x
