...
Puppeteer.prototype.update = function(world){
  ...
  if( this.level < 0 || !world.runLevels || world.over || world.paused )
    return;
  ...
  if( level.extras.nextFrame < 0 ){
    level.extras.nextFrame = level.extras.frame + world.rand.range(Puppeteer.MIN_EXTRA_SPAWN_TIME,Puppeteer.MAX_EXTRA_SPAWN_TIME);
  }
  ...
  if( level.extras.frame++ > level.extras.nextFrame ){
    ...
    // dumb count since some bodies are removed but
    ...
    if( world.extras.length !== countExtras )
      console.warn('still have removed extras in world.extras');
  ...
  for(var i=world.obstacles.length; i<level.obstacles.length;i++){
    actions.createNextObstacle(world,level)
  }
}
