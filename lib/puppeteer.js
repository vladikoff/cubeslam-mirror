...
Puppeteer.prototype.add = function(level,i){
  level = Puppeteer.parse(this.actions,level,this.levels[i]);
  if( typeof i == 'number' ){
    this.levels[i] = level;
  } else {
...
Puppeteer.prototype.update = function(world){
  ...
  for(var i=this.action; i < level.actions.length; i++ ){
    ...
    // make sure it exists
    if( this.actions.hasOwnProperty(act.action) ){
      console.log('called action "%s" at frame %s',act.action,world.frame,act.params)
      this.actions[act.action].apply(this.actions,act.params);
 ...
 */
Puppeteer.load = function(actions,levels){
  return new Puppeteer(actions).load(levels)
}
 ...
 * Used by #add().
 */
Puppeteer.parse = function(actions,level,into){
