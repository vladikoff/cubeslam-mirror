...
function Renderer(element){
  ...
  this.paddlePlayer = createPaddle(this.element, 'p1');
  this.paddleCPU = createPaddle(this.element, 'p2');
}
...
Renderer.prototype = {
  ...
  triggerEvent: function(id, paramObj){
    ...
    switch(id) {
      ...
      case 'gameStart':
        ...
        break;
      case 'toggleFog':
        toggleFog(paramObj.active, id==1 )
        break;
      case 'puckBounce':
        if(window.navigator.vibrate)
          window.navigator.vibrate(200)
        // console.log( paramObj.puck.index );
        // puckHit(this.pucks.get(paramObj.puck.index))
        break;
    }
  },
  ...
  render: function(world, alpha){
    ...
    for(var i=0; i < world.pucks.values.length; i++){
      ...
      var transform = 'rotateX(-90deg) translateX('+(x-23)+'px) translateY(-50%) translateZ('+(y+23)+'px) scale(1.5)'
      ,   puckClass = ('puck puck-' + (bg < 10 ? '0'+bg : bg))

      div.style.webkitTransform = transform;

      if( puck.data.ghostball )
        puckClass += puck.data.ghostball == 1 ? ' ghostball' : ''
      div.setAttribute('class', puckClass);
    }
    ...
    for(var i=0; i < world.extras.values.length; i++){
      var extra = world.extras.values[i]
        ...
        x = x / w * this.width;
        y = y / h * this.height;
      div.style.webkitTransform = 'rotateX(-90deg) translateX('+(x-31)+'px) translateY(-50%) translateZ('+y+31+'px) scale(1.4)';
    }
  ...
  movePaddle: function(paddle, element, player, alpha) {
    ...
    if( paddle ) {
      ...
      element.style.webkitTransform = 'rotateX(-90deg) translateX('+(x-78)+'px) translateY(-50%) translateZ('+y+'px) scale(1.5)';

      // TODO update the sprite depending on x
      element.setAttribute('class', (player + '-00' + (bg < 10 ? '0'+bg : bg)));
      var gfx = element.children[0];
      gfx.style.webkitTransform = 'scaleX('+scaleX+')';
    }
  }
}

var piece = $('<div class="empty"><div class="icon"></div></div>')
  , puck = $('<div class="puck puck-15"><div class="gfx"></div></div>')
  , paddle = $('<div class="paddle player"><div class="gfx"></div></div>')
  , shield = $('<div class="shield"></div>')
  , extra = piece.clone()

function createPaddle(parent, player){
  debug('create paddle');
  return paddle.clone().attr('id', player).appendTo($(parent).find('.arena'))[0];
}
function createPuck(parent,body){
  debug('create puck',body.index);
  return $(parent).find('.empty').first()
    .removeAttr('style')
    .addClass('puck puck-15')[0]
}
...
function createExtra(parent,body){
  ...
  var extraType = body.data.id.replace(' ', '-');
  return $(parent).find('.empty').first()
    .removeAttr('style')
    .addClass('extra '+ extraType)[0]
}
...
function createShield(parent,body){
  ...
  var s = $(parent)
    ...
    .find('.shield-'+parseInt(body.data.index+1)).show();
  cssEvent(s[0]).off('end');
  setTimeout( function(){
    s.addClass('visible');
  }, 4)
  return s[0]
...
function removeShield(shield){
  debug('remove shield');
  if( !shield ){
    console.warn('attempted to remove non-existing shield (css renderer)')
    return;
  }

  $(shield).removeClass('visible').addClass('hit');
  cssEvent(shield).once('end',function(){
...
function puckHit(puck){
  // var p = $(puck).css('opacity', .6);
  // setTimeout( function(){
  //   p.removeAttr('style');
  // }, 50)
}
