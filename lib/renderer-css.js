...
Renderer.prototype = {
  ...
  render: function(world, alpha){
    ...
    for(var i=0; i < world.pucks.values.length; i++){
      ...
      var bg = parseInt(x/w * 31)
      x = x / w * this.width;
      y = y / h * this.height;
      // TODO position the div
      div.style.webkitTransform = 'rotateX(-90deg) translateX('+(x-23)+'px) translateY(-50%) translateZ('+(y+23)+'px)';
    ...
    for(var i=0; i < world.extras.values.length; i++){
      var extra = world.extras.values[i]
        ...
        x = x / w * this.width;
        y = y / h * this.height;
      div.style.webkitTransform = 'rotateX(-90deg) translateX('+(x-64)+'px) translateY(-50%) translateZ('+y+'px) scale(1.2)';
    }
  ...
  movePaddle: function(paddle, element, player, alpha) {
    ...
    if( paddle ) {
      ...
      element.style.webkitTransform = 'rotateX(-90deg) translateX('+(x-78)+'px) translateY(-50%) translateZ('+y+'px) scale(1.1)';
      // TODO update the sprite depending on x
...
Bear.prototype = {
  change: function(to){
    ...
    switch(to) {
      ...
      case 'jawdrop':
        this.expressions = [];
        var exp = ['jawdrop-1', 'jawdrop-2', 'jawdrop-3', 'jawdrop-4', 'jawdrop-5', 'jawdrop-6', 'jawdrop-6', 'jawdrop-6'];
        this.expressions.push.apply(this.expressions, exp.concat())
        this.expressions.push.apply(this.expressions, exp.concat().reverse())
        break;
      case 'flirt':
