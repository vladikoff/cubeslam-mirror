...
Renderer.prototype = {
  ...
  render: function(world, alpha){
    ...
    if( world.paddles.has(world.me.paddle) ) {
      this.movePaddle(world.paddles.get(world.me.paddle), this.paddlePlayer, alpha)
      this.movePaddle(world.paddles.get(world.opponent.paddle), this.paddleCPU, alpha)
    }
  ...
  movePaddle: function(paddle, element, alpha) {
    ...
    if( paddle ) {
      ...
      // TODO update the sprite depending on x
      element.setAttribute('class', ('p1 p1_00' + (bg < 10 ? '0'+bg : bg)));
    }
  }
}

var puck = $('<div class="puck"></div>')
