...
Localization.prototype.nextLanguage = function(){
  // find the next accepted language that is available
  var sortedAvailable = [];
  for(var i=0; i<this.acceptedLanguages.length; i++){
    ...
    if( isAvailable ){
      sortedAvailable.push(acc.value);
    }
  }

  // find the index of the current language
  var curr = sortedAvailable.indexOf(this.currentLanguage);
  var next = sortedAvailable[(curr+1)%sortedAvailable.length];
...
Localization.prototype.load = function(fn){
  ...
  batch.end(function(){
    ...
    self.register(self.defaultLanguage);
    fn && fn();
  });
