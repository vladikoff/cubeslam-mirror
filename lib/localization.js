...
var localization = {
  ...
  init: function(acceptLanguage) {
    ...
    var acceptLanguage = acceptLanguage.split(',');
    ...
    $.ajax({
      'url': '/localization/localization.arb', // All arb stuff should go in this file.
      'dataType': 'text',
  ...
  setLanguage: function(language) {
    ...
    if (language != this.currentLanguage) {
      ...
      if (availLanguages.indexOf(language) != -1) {
        ...
        console.log('Nr of namespaces: ' + this.languages[this.currentLanguage].length);
        for (var i = 0; i < this.languages[this.currentLanguage].length; i++) {
          console.log('Applying namespace: ' + this.languages[this.currentLanguage][i]);
