...
see.abort = function(){
  console.log('abort',queue,stack,active)
...
function pop(){
  var pathname = stack.pop()
    , state = active = lookup.pop();

  // run
  if( state && typeof state.leave == 'function' ){
    // async
    if( state.leave.length >= 2 ){
      console.log('pop() async',context.pathname)
    ...
    // sync
      ...
      console.log('pop() sync',context.pathname)
      state.leave(context);
      see.emit('leave',context);
      state.cleanup && state.cleanup(context);
      active = null;
  ...
  // no leave
    ...
    console.log('pop() no leave',context.pathname)
    see.emit('leave',context);
    state.cleanup && state.cleanup(context);
    active = null;
    go();
...
function push(){
  ...
  if( state && typeof state.enter == 'function' ){
    ...
    if( state.enter.length >= 2 ){
      console.log('push() async',context.pathname)
    ...
    // sync
      ...
      console.log('push() sync',context.pathname)
      see.emit('enter',context);
      state.enter(context);
      active = null;
      stack.push(context.pathname)
  ...
  // no enter
    ...
    console.log('push() no enter',context.pathname)
    see.emit('enter',context);
    active = null;
    push()
