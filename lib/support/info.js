...
exports.dataChannel = function(dataChannel){
  ...
  $("#debug-info").append('<li class="data-channel">'+ts()+'<span class="label">'+dataChannel.label+'</span>'+dataChannel._localId+' -> '+dataChannel._remoteId + '</li>');
}

exports.stateHistory = function(source,state){
  debug('state history',source,state)
  $("#debug-info").append('<li class="state-history '+source+'" title="'+source+'">'+ts()+state+'</li>');
}

exports.hostStatus = function(bool){
  debug('host status',bool ? 'Host' : 'Guest')
  $("#debug-info").append('<li class="host-status '+(bool ? 'host' : 'guest')+'">'+ts()+ (bool ? 'Host' : 'Guest') + '</li>');
}
