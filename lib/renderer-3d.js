...
Renderer.prototype = {
  ...
  createNature: function(){
      ...
      for (var i = 0; i < len; i++) {
        ...
        var uvY = ((point.y+1500) / 4000)

        var colorInit = new THREE.Color( 0x1a99db );
        var color = colorInit.getHSV();

        //color.setRGB( colorInit.r-uvY*0.3, colorInit.g-uvY*0.3, colorInit.b-uvY*0.3 );
        colorInit.setHSV( color.h, color.s, color.v - uvY*0.2);

        geometry.colors[i] = colorInit;
  ...
  render: function(world, alpha){
    ...
    for(var i=0; i < world.pucks.length; i++){
      ...
      mesh.position.x = (puck.current.x*fw-hw)*((world.host)?1:-1);
      mesh.position.z = (puck.current.y*fh-hh)*((world.host)?1:-1);
    ...
    var mesh = this.paddleMeshes[0];
    var paddle = world.host ? world.players.a.paddle : world.players.b.paddle;
    mesh.position.x = (paddle.x*fw-hw)*(world.host?1:-1);
    mesh.position.z = (settings.data.arenaHeight*.5-settings.data.paddleDepth*.5)
    mesh.scale.x = (paddle.width*fw)/100;

    //base camera position on users movement
    this.camera.position.x = 40*(paddle.x-0.5);

    mesh = this.paddleMeshes[1];
    paddle = !world.host ? world.players.a.paddle : world.players.b.paddle;
    mesh.position.x = (paddle.x*fw-hw)*(world.host?1:-1);
    mesh.position.z = -(settings.data.arenaHeight*.5-settings.data.paddleDepth*.5)
    mesh.scale.x = (paddle.width*fw)/100;
