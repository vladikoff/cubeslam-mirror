...
Renderer.prototype = {
  ...
  createEnvironment: function(){
    var envGeo = new THREE.PlaneGeometry(8000,3000,20,20),
        vertices = envGeo.vertices;

    for (var i = vertices.length - 1; i >= 0; i--) {
     vertices[i].z = Math.random()*200;
    };

    var envMesh = new THREE.Mesh(envGeo,new THREE.MeshLambertMaterial({wireframe:false,color:0xffffff}) )
    envMesh.position.y = -200;
    envMesh.position.z = 3000;
    envMesh.rotation.x = -Math.PI*.6;
  ...
  render: function(alpha){
    ...
    for(var i=0; i < this.paddles.length; i++ ){
      ...
      mesh.position.x = paddle.x/window.innerWidth*hw;
