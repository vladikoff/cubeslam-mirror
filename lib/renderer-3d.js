...
*/
  ...
  createArena: function(){
    ...
    this.tableUniforms = {
      ...
      scale:  { type: "v2", value: new THREE.Vector2(18, 30 ) }
    }
    ...
    var columnValues = new Array(18);
    for (var i = columnValues.length - 1; i >= 0; i--) {
      columnValues[i] = Math.floor(Math.random()*2);
    };

    this.guestShieldUniforms = {
      uColumns: { type:"f", value:columnValues.length},
    ...
    this.hostShieldUniforms = {
      uColumnValues: { type: "fv", value: columnValues},
      uColumns: { type:"f", value:columnValues.length},
      uBrightness:  { type: "f", value: 1.0 },
      uColor: { type: "c", value: new THREE.Color( settings.data.ballColor ) }
    }
    ...
    var hSGeo = new THREE.CubeGeometry(100,sideH,4,1,1,1)
    hSGeo.sides = { px: false, nx: false, py: true, ny: false, pz: true, nz: true };
