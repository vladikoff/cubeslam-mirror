...
Renderer.prototype = {
  ...
  createPaddles: function(){
    ...
    var opponentPaddleGeo = new THREE.CubeGeometry( 100, 60, 20 )
      , opponentPaddleMat = new THREE.MeshPhongMaterial( { color:0x999999, transparent:false })
      , opponentPaddle = new THREE.Mesh( opponentPaddleGeo, opponentPaddleMat );
    opponentPaddle.position.z = this.bounds.t-halfDepth+20;
    opponentPaddle.position.y = this.surfaceY;
    this.gameContainer.add(opponentPaddle);

    var userPaddleGeo = new THREE.CubeGeometry( 100, 30, 20 )
      , userPaddleMat = new THREE.MeshPhongMaterial( { color:0x999999, transparent:false })
      , userPaddle = new THREE.Mesh( userPaddleGeo, userPaddleMat );
    userPaddle.position.z = this.bounds.b-halfDepth+this.ballRadius;
...
*/
  ...
  createArena: function(){
    ...
    this.guestShieldUniforms = {
      uiBrightness:  { type: "f", value: 1.0 },
      uiColor: { type: "c", value: new THREE.Color( settings.data.ballColor ) }
    }
    ...
    this.hostCube = new THREE.Mesh( new THREE.CubeGeometry(100,100,500,1,1,1), new THREE.MeshPhongMaterial({color:0x000000}) );
    this.hostCube.position.z = this.bounds.b - 200;
    this.hostCube.position.y = this.surfaceY - 300;
  ...
  render: function(alpha){
    ...
    if( this.guestShieldUniforms.uiBrightness.value > 0.4 ) this.guestShieldUniforms.uiBrightness.value *= 0.98;
    if( this.hostShieldUniforms.uiBrightness.value > 0.4 ) this.hostShieldUniforms.uiBrightness.value *= 0.98;
    ...
    this.hostCube.scale.set(fw/100,videoHeight,1)
    this.hostCube.position.z = (hh+250) * ((this.inverted)?-1:1);
    this.hostCube.position.y = this.surfaceY + videoHeight*.5*100;

    this.hostShieldMesh.scale.set(fw/100,1,1);
    this.hostShieldMesh.position.z = (hh-10) * ((this.inverted)?-1:1);

    this.guestShieldMesh.scale.set(fw/100,1,1);
    this.guestShieldMesh.position.z = (hh-15) * ((this.inverted)?1:-1);
    ...
    for(var i=0; i < this.pointMasses.length; i++){
      ...
      if( mesh.material.color.getHex() != settings.data.ballColor) {
        this.guestShieldUniforms.uiColor.value = new THREE.Color( settings.data.ballColor )
        this.hostShieldUniforms.uiColor.value = new THREE.Color( settings.data.ballColor )
  ...
  testBallHit : function(at){
    ...
    if( at.y <= this.bounds.t/settings.data.arenaHeight ){
          ...
          if( at.x >= x - hw && at.x <= x + hw ){
            ...
            //guest miss
            this.guestShieldUniforms.uiBrightness.value = 2.4;
          }
          ...
          if( at.x >= x - hw && at.x <= x + hw ){
            ...
            this.hostShieldUniforms.uiBrightness.value = 2.4;
          }
