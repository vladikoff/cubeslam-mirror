...
Renderer.prototype = {
  ...
  loadShaders: function() {
    this.shaderLibrary = require('./shaders')
    this.geometryLibrary = require('./geometry')
    this.geometryLibrary[Renderer.EXTRA_EXTRABALL] = this.geometryLibrary.extra_plus
  ...
  createMaterials: function(){
    ...
    var uniforms = {
      ...
      scale: { type: "v2", value: new THREE.Vector2(Renderer.ARENA_COLUMNS , 26 ) }
    }
  ...
  createTerrain: function(){
      ...
      // var geometry = new THREE.GeometryLoader().parse(this.geometryLibrary["terrain1"])
      var geometry = this.geometryLibrary["terrain1"];
  ...
  render: function(world, alpha){
    ...
    this.time += 0.01;
/*
    this.camera.position.x = Math.cos(this.time) * 1500;
    this.camera.position.z = Math.sin(this.time) * 1500;
    this.camera.lookAt(this.cameraTarget)
    this.camera.position.y = 3400;*/
    ...
    if( settings.data.puckColor != this.playerA.shieldUniforms.uColor.value.getHex()) {
        this.playerA.shieldUniforms.uColor.value.setHex( settings.data.puckColor )
        this.playerB.shieldUniforms.uColor.value.setHex( settings.data.puckColor )
    }
  ...
  updateHits : function(world){
    ...
    this.materialLibrary["arenaGrid"].uniforms.points.value.y = hits.length
    ...
    this.materialLibrary["arenaGrid"].uniforms.points.value.x = hits.length
    for (var i = hits.length - 1; i >= 0; i--) {
      targetColumn = Math.floor(hits[i]*Renderer.ARENA_COLUMNS);
