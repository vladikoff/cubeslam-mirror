...
Renderer.prototype = {
  ...
  render: function(alpha){
    ...
    this.ballMesh.position.x = puck.current.x*fw-hw;
    this.ballMesh.position.z = puck.current.y*fh-hh;
    ...
    if( this.videoMaterialTexture && this.videoMaterialTexture.image.readyState === this.videoMaterialTexture.image.HAVE_ENOUGH_DATA) {
  ...
  setVideoTexture : function(canvas) {
    ...
    if( !this.videoInitiated ) {
      ...
      var videoGeo = new THREE.PlaneGeometry(tableBB.xmax-tableBB.xmin,tableBB.ymax-tableBB.ymin);
      var videoMat = new THREE.MeshBasicMaterial({
        map : this.videoMaterialTexture,
        side: THREE.DoubleSide
      });

      //video screen
      var videoPlane = new THREE.Mesh(videoGeo, videoMat);
      videoPlane.position.x = 0;
      videoPlane.position.y =  0;
      videoPlane.position.z = this.bounds.b;
