...
Network.prototype.setupRemote = function(opts){
  ...
  sync.on('done',function(){
    ...
    console.error('time sync latency',this.latency)
  ...
  this.remote.on('connected',function(e){ // a peer has joined
    ...
    //   console.log('netchan latency',latency)
    //   ctx.latency = latency;
...
Network.prototype.send = function(msg){
  ...
  if( this.game && this.game.open ){
    buffer.push(msg)
    buflen += msg.byteLength

    // throttle to ~15fps
    if( !this.flushTimeout ){
      this.flushTimeout = setTimeout(this.flush.bind(this),66) // ~15 fps
    }
  } else {
