...
Network.prototype.checkReady = function(){
  if(this.ready){
    console.log('checkReady - previously ready')
    return true;
  }
  if(!this.connected){
    console.warn('checkReady - not connected')
    return false;
  }
  if(!this.remote.channels.game){
    console.warn('checkReady - no game data channel')
    return false;
  }
  if(!this.context.latency){
    console.warn('checkReady - no latency results. has time sync started?')
    return false;
  }
  if(!this.context.multiplayer){
    console.warn('checkReady - not multiplayer')
    return false;
  }
  if(this.pathname === null){
    console.warn('checkReady - no remote pathname received yet')
    return false;
  }
  if(this.remote.connection.getLocalStreams().length === 0){
    console.warn('checkReady - no local video')
    return false;
  }
  if(this.remote.connection.getRemoteStreams().length === 0){
    console.warn('checkReady - no remote video')
    return false;
  }
  this.ready = true;
  this.emit('ready')
  console.log('checkReady - network is now READY!')
}
