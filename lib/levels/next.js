var settings = require('../settings')

module.exports = {
  name: 'Level 1',
  description: 'Just starting eh?',

  // speeds are multipliers of settings.data.unitSpeed
  cpu: {
    speed: 1,
    // what else can we control with the AI?
    // - reaction time? (z distance from the ball until movement)
    // - max speed?
    // - shakiness? (random jumps back and forth)
  },

  // speeds are multipliers of settings.data.unitSpeed
  // these are the starting speeds of the puck. then each
  // puck will have different speeds depending on the times
  // they've bounced against a paddle
  puck: {
    // unit * (speed+speedup*bounces)
    // 20 * (1+0) > 20 * (1+.2) > 20 * (1+.4)
    // ex. 20 > 20 * 1.2 > 20 * 1.4 > ... > 20 * 4
    speed: 1,
    speedup: .1,
    maxspeed: 4
  },

  player: {
    // the number of slots in the shield array of the
    // world.players.a/b. which will generate a shield
    // "physically" and visually.
    // must be divisable with settings.data.arenaColumns
    shields: 3
  },

  // extras will be added every 5-10 seconds and will be placed
  // at (pseudo-)randomly on the defined positions below. the
  // `available` list here is the available ones in this level.
  // the full list of available extras can be found in
  // lib/actions/extra.js
  extras: {
    available: [
      //{id: 'bulletproof', duration: 10}, // duration in seconds (buggy)
      //{id: 'mirrored controls', duration: 10}, // duration in seconds
      //{id: 'random'},
      //{id: 'fog', duration: 10},
      //{id: 'extra life'},
      //{id: 'ghost ball'},
      {id: 'fireball'},
      //{id: 'multiball'}
    ],
    // ~1700x2455
    positions: [
      {x: 200, y: 200},
      {x: 850, y: 1700},
      {x: 850, y: 700},
      {x: 200, y: 2000},
      {x: 1400, y: 200},
      {x: 1400, y: 2000}
    ]
  },

  // obstacles will be added during the game as soon as the ball
  // is not in the way. and it will stay until the end of the level.
  // the full list of available extras can be found in
  // lib/actions/obstacle.js
  obstacles: [
   //{id: 'triangle-left', x: 0+ settings.data.unitSize*2, y: settings.data.arenaHeight*.5},
   //{id: 'triangle-right', x: settings.data.arenaWidth- settings.data.unitSize*2, y: settings.data.arenaHeight*.5}
   //{id: 'diamond', x: settings.data.arenaWidth*.5, y: settings.data.arenaHeight*.5}
   //{id: 'hexagon', x: settings.data.arenaWidth*.5, y: settings.data.arenaHeight*.5}
   //{id: 'block-rect-6x4', x: settings.data.unitSize*3, y: settings.data.arenaHeight*.5},
   //{id: 'block-rect-6x4', x: settings.data.arenaWidth-settings.data.unitSize*3, y: settings.data.arenaHeight*.5}

  /* {id: 'block-rect-2x2-destroy', x: settings.data.unitSize, y: settings.data.arenaHeight*.5},
  // {id: 'block-rect-2x2-destroy', x: settings.data.unitSize*5, y: settings.data.arenaHeight*.5},
   {id: 'block-rect-2x2-destroy', x: settings.data.unitSize*9, y: settings.data.arenaHeight*.5},
  // {id: 'block-rect-2x2-destroy', x: settings.data.unitSize*13, y: settings.data.arenaHeight*.5},
   {id: 'block-rect-2x2-destroy', x: settings.data.unitSize*17, y: settings.data.arenaHeight*.5}
   */

   /*{id: 'block-rect-2x2-destroy', x: settings.data.unitSize*4, y: settings.data.arenaHeight*.5 + settings.data.unitSize*7},
  // {id: 'block-rect-2x2-destroy', x: settings.data.unitSize*5, y: settings.data.arenaHeight*.5},
   {id: 'block-rect-2x2-destroy', x: settings.data.arenaWidth*.5, y: settings.data.arenaHeight*.5},
  // {id: 'block-rect-2x2-destroy', x: settings.data.unitSize*13, y: settings.data.arenaHeight*.5},
   {id: 'block-rect-2x2-destroy', x: settings.data.arenaWidth - settings.data.unitSize*4, y: settings.data.arenaHeight*.5 - settings.data.unitSize*7}
   */
   
  {id: 'block-rect-2x8', x: settings.data.arenaWidth*.5 - settings.data.unitSize*4, y: settings.data.arenaHeight*.5},
  {id: 'block-rect-2x8', x: settings.data.arenaWidth*.5 + settings.data.unitSize*4, y: settings.data.arenaHeight*.5}
   

  ]
}
